...
func TestT1269BorrowParamAliasClonesResult(t *testing.T) {
	...
	codegentest.AssertContains(t, ir, "alias.borrow.clone")
	codegentest.AssertContains(t, ir, "alias.borrow.cont")
...
func TestT1269OwnedLocalSourceUsesDupNotBorrow(t *testing.T) {
	ownedLocal := codegentest.GenerateIR(t, `
		type Money { int cents; }
		ident(Money[] v) Money[] { return v; }
		main() {
	...
	baseline := codegentest.GenerateIR(t, `
		type Money { int cents; }
		ident(Money[] v) Money[] { return v; }
		main() {
			Money[] xs = [Money(cents: 1)];
	...
	codegentest.AssertContains(t, ownedLocal, "alias.dup")
	codegentest.AssertContains(t, ownedLocal, "vecdup.copy")
...
func TestT1269HeapUserTypeBorrowAliasClonesResult(t *testing.T) {
	...
	codegentest.AssertContains(t, ir, "heapdup")
}
