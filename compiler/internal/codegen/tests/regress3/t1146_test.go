...
func TestT1146_MoveArgDups(t *testing.T) {
	ir := codegentest.GenerateIR(t, t1146Decls+`
		arg_consume() int {
		...
		main() { _ := arg_consume(); }
	`)
	fn := codegentest.ExtractFunction(ir, "__user.arg_consume")
	if fn == "" {
...
func TestT1146_ConstructorArgDups(t *testing.T) {
	ir := codegentest.GenerateIR(t, t1146Decls+`
		...
		main() { _ := ctor_move(); }
	`)
	fn := codegentest.ExtractFunction(ir, "__user.ctor_move")
	if fn == "" {
...
func TestT1146_ReturnDups(t *testing.T) {
	...
	`)
	fn := codegentest.ExtractFunction(ir, "__user.relay")
	if fn == "" {
...
func TestT1146_SynthDropBindDups(t *testing.T) {
	ir := codegentest.GenerateIR(t, `
		...
		main() { _ := bag_bind(); }
	`)
	fn := codegentest.ExtractFunction(ir, "__user.bag_bind")
	if fn == "" {
