...
| B0042 | Formatter strips `\{` escape — `promise format` converts `"\{"` back to `"{"`, re-triggering string interpolation. The `\{` escape works correctly in the lexer/parser/codegen but is not preserved by the formatter. Blocks removal of `_json_lbrace()` workaround in json module. | High |
...
|---|------|-----------|----------|
| T0001 | `JsonValue` use `map[string, JsonValue]` for Object variant instead of parallel arrays | B0030 (`map[K, Enum]` codegen fix) | High |
