...
func TestT1117_DirectRefMapReadDupsArc(t *testing.T) {
	...
	codegentest.AssertContains(t, fn, "atomicrmw add")
}
...
func TestT1117_StructRefMapReadDups(t *testing.T) {
	ir := codegentest.GenerateIR(t, `
		...
		main() { probe(); }
	`)
	fn := codegentest.ExtractDefine(ir, `"Map[int, RefBox].[]"`)
	if fn == "" {
...
func TestT1117_EnumRefBindReadDups(t *testing.T) {
	ir := codegentest.GenerateIR(t, `
		...
		main() { probe(); }
	`)
	fn := codegentest.ExtractDefine(ir, `"Map[int, RefEnum].[]"`)
	if fn == "" {
		t.Fatalf("Map[int, RefEnum].[] not found in IR")
	}
	codegentest.AssertContains(t, fn, "enumdup.done")
	codegentest.AssertContains(t, fn, "arcdup.inc")
}
...
func TestT1117_EnumRefInlineReadNoDup(t *testing.T) {
	...
	codegentest.AssertNotContains(t, fn, "enumdup")
}
...
func TestT1117_StructWeakMapReadDups(t *testing.T) {
	ir := codegentest.GenerateIR(t, `
		...
		main() { probe(); }
	`)
	fn := codegentest.ExtractDefine(ir, `"Map[int, WeakBox].[]"`)
	if fn == "" {
