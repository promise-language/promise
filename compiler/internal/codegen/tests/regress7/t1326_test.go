...
func TestT1326IfArmNestedCallArgDrains(t *testing.T) {
	...
	`)
	fn := codegentest.ExtractFunction(ir, "__user.branch_if")
	if fn == "" {
...
func TestT1326MatchExprArmNestedCallArgDrains(t *testing.T) {
	...
	`)
	fn := codegentest.ExtractFunction(ir, "__user.branch_match")
	if fn == "" {
...
func TestT1326DirectCtorArmNoDrain(t *testing.T) {
	...
	`)
	fn := codegentest.ExtractFunction(ir, "__user.branch_direct")
	if fn == "" {
...
func TestT1326ArmTailDivergesViaBreak(t *testing.T) {
	...
	`)
	if codegentest.ExtractFunction(ir, "__user.arm_break") == "" {
