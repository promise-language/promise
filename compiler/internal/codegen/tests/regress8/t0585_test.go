...
func TestT0585_WrapBorrowedParamClearsBindingDropFlag(t *testing.T) {
	ir := codegentest.GenerateIR(t, `
		...
		main() {}
	`)
	body := codegentest.ExtractFunction(ir, "__user._caller")
	if body == "" {
...
func TestT0585_WrapOwnedLocalPropagatesDropFlag(t *testing.T) {
	ir := codegentest.GenerateIR(t, `
		...
		main() { _helper(); }
	`)
	body := codegentest.ExtractFunction(ir, "__user._helper")
	if body == "" {
...
func TestT0585_ConsumeParamHasDropFlag(t *testing.T) {
	ir := codegentest.GenerateIR(t, `
		...
		main() {}
	`)
	body := codegentest.ExtractFunction(ir, "__user._consume")
	if body == "" {
