...
| B0044 | `go extern_func()` panics in codegen: `store operands are not compatible: src=void; dst=i8**` at `genGoCallExpr` (expr.go). Extern functions use sret ABI (void return + first param is sret pointer), but `genGoCallExpr` assumes the call returns a value to store into the task result slot. Affects any `go` expression calling an extern-attributed function that returns a non-void Promise type. Workaround: wrap the extern in a regular Promise function. | 5c | Medium |
...
| ~~B0027~~ | ~~`os.execute` reads stdout/stderr sequentially — deadlock if child writes >64KB to stderr while stdout pipe is not drained~~ — **Fixed.** Decomposed `pal_execute` into `pal_spawn` + `pal_read_pipe` + `pal_wait_pid`. `execute()` in os.pr reads both pipes concurrently using goroutines (`go _read_pipe(stderr_fd)` for stderr, `_read_pipe(stdout_fd)` for stdout). Each pipe read releases the scheduler P via enter/exit_syscall. | E | ~~Medium~~ Resolved |
