...
func TestT1311MethodDiscardFreshTempClearsResult(t *testing.T) {
	...
	`)
	fn := codegentest.ExtractFunction(ir, "__user.method_discard_freshtmp")
	if fn == "" {
...
func TestT1311MethodInlineFreshTempClearsResult(t *testing.T) {
	...
	`)
	fn := codegentest.ExtractFunction(ir, "__user.method_inline_freshtmp")
	if fn == "" {
...
func TestT1311MethodDiscardFreshReturnEmitsGuardDistinctPtr(t *testing.T) {
	...
	`)
	fn := codegentest.ExtractFunction(ir, "__user.method_fresh_return")
	if fn == "" {
...
func TestT1311MethodDiscardFreshTempTwoArgsEmitsPerArgGuards(t *testing.T) {
	...
	`)
	fn := codegentest.ExtractFunction(ir, "__user.method_two_freshtmp")
	if fn == "" {
...
func TestT1311GenericMethodDiscardFreshTempClearsResult(t *testing.T) {
	...
	`)
	fn := codegentest.ExtractFunction(ir, "__user.generic_freshtmp")
	if fn == "" {
...
func TestT1311MethodDiscardNoArgNoGuard(t *testing.T) {
	...
	`)
	fn := codegentest.ExtractFunction(ir, "__user.method_none")
	if fn == "" {
		t.Fatalf("could not extract @__user.method_none from IR:\n%s", ir)
	}
	codegentest.AssertContains(t, fn, "@__promise_structural_drop")
	codegentest.AssertNotContains(t, fn, "struct.tmp.freshalias.clear")
}
