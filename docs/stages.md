...
| `as!` on optionals (`T? as! T`) — sema accepts it but codegen panics (`store operands not compatible: src={i1, T}; dst=T*`). The cast should extract T and panic on none. Fixing this unblocks `serializable` decode for nested user-defined types. | Codegen | High |
