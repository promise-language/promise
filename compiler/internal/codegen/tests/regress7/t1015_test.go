...
func TestT1015_EnumMemberCompoundOpDispatch(t *testing.T) {
	...
	codegentest.AssertContains(t, body, `@"Tag.+"(i8*`)
	...
	codegentest.AssertContains(t, body, "@Tag.drop(i8*")
}
...
func TestT1015_EnumSliceCompoundOpDispatch(t *testing.T) {
	...
	codegentest.AssertContains(t, body, `@"Box.[:]"(i8*`)
	codegentest.AssertContains(t, body, `@"Tag.+"(i8*`)
...
func TestT1015_GenericEnumCompoundOpDispatch(t *testing.T) {
	...
	codegentest.AssertContains(t, body, `@"Box[int].+"(i8*`)
	codegentest.AssertNotContains(t, body, `@"Box.+"(i8*`)
	// Drop-old fires through the mono'd drop.
	codegentest.AssertContains(t, body, `@"Box[int].drop"(i8*`)
}
...
func TestT1015_GenericEnumCompoundOpInGenericMethod(t *testing.T) {
	ir := codegentest.GenerateIR(t, `
		...
		main() { caller(); }
	`)
	body := codegentest.ExtractFunction(ir, `"Box[int].combine"`)
	if body == "" {
	...
	codegentest.AssertContains(t, body, `call %"promise_Box[int]_enum" @"Box[int].+"(i8*`)
}
