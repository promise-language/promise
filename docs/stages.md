...
- **Result struct**: Failable with return type (e.g., `foo!() T`) → `{ i1, T, i8* }` (tag, ok value, error pointer). Void failable (e.g., `foo!()`) → `{ i1, i8* }` (tag, error pointer). Tag: `i1 false` = Ok, `i1 true` = Error.
...
- **Naming convention**: `Box[int]`, `Pair[int, string]`, `Option[int]` for types. `identity[int]` for generic functions. Bracket notation mirrors Promise source syntax — no collision with user identifiers since `[`/`]` are not valid in Promise names. The llir/llvm library auto-quotes these in LLVM IR (e.g. `@"Box[int].push"`).
- **Core infrastructure** (`mono.go`): `monoContext` struct, `monoName`/`typeArgStr`/`monoFuncName` mangling, `collectMonoInstances`/`collectMonoFuncInstances` deduplication. Layout computation: `computeMonoUserTypeLayout` (four-struct ABI with substituted field types), `computeMonoEnumLayout` (tagged union with substituted variant fields). Method/function codegen: `declareMonoMethods`/`defineMonoMethods`/`declareMonoFuncs`/`defineMonoFuncs` with `typeSubst`+`monoCtx` scoping and `defer`-based state cleanup.
...
- **Failable `new`**: `new!(~this, params)` — codegen wraps result in `{ i1, T, i8* }` result struct with branch/merge/phi for error vs ok paths at the constructor call site.
...
- **Std as catalog module**: `std` is a regular embedded catalog module (`modules/std/promise.toml`, `catalog.toml`). Every file auto-receives an injected `use std as _;` glob import — no special `stdScope` parent chain. Std compiles through the normal `CheckWithTarget` path. Non-native types (error, Map, Range, Iterator, Stream) are regular module types; `populateUniverseTypes()` sets global identity pointers (TypError, TypMap, etc.) after std's declare pass. `use std;` / `use std as s;` still work for qualified access.
...
- **Test suite**: 2563 tests across 215 files — `tests/e2e/` (language features), `tests/std/` (standard library), `tests/concurrency/` (scheduler, channels, select, panic recovery, stress tests), `tests/modules/` (module system e2e), `tests/value_types/` (pure value types), `modules/*/` (catalog module tests)
...
| ~~Stack overflow detection (guard page + SIGSEGV handler)~~ | ~~Done~~ |
...
| ~~Generic free-function/method-instance ownership~~ — **Fixed.** `sort[int]`, `scan[int]`, `sort[Priority]`, `Iterator[int].map[int]` etc. were module-owned but not instance-owned, so `SplitModuleIRs` left their bodies in the module IR, making it unstable across compilations and causing undefined-symbol linker errors when a cached module BC was reused for a different caller. Fixed in `declareMonoFuncs` and `declareMonoMethodInstances` (mono.go): when `compilingModule != ""`, now also adds `instanceOwnedFuncs[name] = name` so each gets its own `.bc`. Module IR is now stable (one cache entry per module version across all 187 test files). | 9 | ~~Medium~~ Resolved |
| ~~Failable auto-propagation in call arguments and assignments~~ — **Fixed.** Failable calls used as arguments (`foo(bar())` where `bar()` is `!`), variadic elements (`sum(bar(), 10)`), and reassignments (`x = bar()`) were silently accepted by sema without auto-propagation. Codegen passed the raw failable result struct `{i1, T, i8*}` instead of extracting the success value `T`, producing wrong results or codegen panics. Fixed in sema: `resolveCallArgs` checks each argument + variadic elements, `resolveImplicitConstructorArgs` checks constructor args, `checkAssignStmt` checks RHS — all via `checkVarDeclFailable`. Fixed in codegen: `genCallArgExpr` helper wraps `genExpr` + auto-propagation check; used in all 27 arg sites in `expr.go` + array literal elements. `genAssignStmt` and compound index assign paths also check `AutoPropagateExprs`. 10 sema tests, 4 codegen IR tests, 21 e2e tests. | 8e | ~~Medium~~ Resolved |

### Codegen Gaps

| Item | Origin | Priority |
|------|--------|----------|
| B0006 | `typeArgStr` only handles `*Named`, `*Enum`, `*Instance`; falls through to `"unknown"` for all other `types.Type` variants (`*Tuple`, `*Optional`, `*SharedRef`, `*MutRef`, `*Pointer`, `*TypeParam`, function types). Tuple type args are syntactically and semantically valid (e.g., `Wrapper[(int, string)]`) and currently produce `Wrapper[unknown]`. Two different tuple args for the same generic → identical mono name → name collision → codegen panic (`store operands are not compatible`). Fix: use `typ.String()` as fallback (all types implement `String()` with Promise source syntax), with explicit `*Instance` case kept to use `monoName()` instead of the Vector-shorthand form from `String()`. Also add a nil guard. All four `typeArgStr` call sites (in `mono.go` and `expr.go`) would benefit from this fix automatically. | 8f | Medium |
| B0007 | ~~Channel send/recv and select allocas are still dynamic~~ **Fixed.** Replaced 5 dynamic `c.block.NewAlloca()` with `createEntryAlloca()`. Root cause of prior dominator violations: `createEntryAlloca` appended to `startBlk` which contained `coro.suspend` — allocas ended up after the suspend, so `coro-split` placed them in the ramp function while uses were in the resume function. Fix: separated initial `coro.suspend` into `coro.init.suspend` block in both `genGoBlock` and `emitSchedulerMain`. | 5c | ~~Medium~~ Done |
| ~~B0008~~ | ~~Blocking select uses polling instead of waiter-list parking~~ — **Done.** Replaced yield-and-retry with SelectWaiterNode (SWN) parking. SWN is G-layout-compatible at fields 0–4 for waiter list interop; field 1=0xFF sentinel distinguishes from G. Per-select mutex provides wake-once semantics via `select_try_wake` CAS on `G.select_case`. Channel wake code uses `promise_waiter_wake_one` (handles both G and SWN). | 5c | ~~Medium~~ Resolved |
| ~~B0009~~ | ~~Fire-and-forget goroutine G struct leak: all `go { }` blocks set `result_ptr` to sentinel `0x1`, preventing goroutine_exit from freeing the G. Only `task[T]` should use the sentinel.~~ **Fixed.** Added `goExprFireAndForget` flag set in `genStmt` for GoExpr statements; `genGoBlock` and `genGoCallExpr` skip sentinel when flag is set, leaving `result_ptr=null` so `goroutine_exit` frees the G. Flag saved/restored in all context-switch sites. | 5c | ~~Low~~ Resolved |
| ~~B0010~~ | ~~Stack overflow detection: deep recursion segfaults with no message.~~ **Fixed.** Guard page via `pthread_attr_setguardsize(4096)` on worker thread stacks. SIGSEGV/SIGBUS handler (`__promise_sigsegv_handler`) prints "fatal: stack overflow" to stderr and calls `_exit(2)`. macOS: `sigaction(SA_ONSTACK)` + per-thread `sigaltstack(64KB)` for reliable delivery on exhausted stacks. Linux: `signal()` best-effort (T0013 tracks proper `sigaction` via raw syscalls). Windows/WASM: no-op stubs (built-in handling). Remaining: T0014 tracks `probe-stack` attribute for large frames that could skip the guard page. | 5c | ~~Medium~~ Resolved |
| ~~B0042~~ | ~~Goroutines spawned in batch test functions never get scheduled.~~ **Fixed.** `GenerateTestMain` now calls `promise_sched_init(num_cpus)` before running tests and `promise_sched_shutdown()` after, so worker Ms exist to pick up goroutines spawned by `go` in batch test functions. Test functions use thread-blocking mode (cond_wait/spin) for channel ops, which works correctly with the scheduler running in background. WASM batch tests with goroutines remain unsupported (cooperative scheduler cannot run goroutines from non-coroutine context). | 5c | ~~High~~ Resolved |
| ~~B0046~~ | ~~`go extern_func()` panics in codegen: `store operands are not compatible: src=void; dst=i8**` at `genGoCallExpr` (expr.go). Extern functions use sret ABI (void return + first param is sret pointer), but `genGoCallExpr` assumes the call returns a value to store into the task result slot~~ — **Fixed.** `genGoCallExpr` now detects extern targets via `c.externs` and generates an auto-wrapper function (`.go_extern_wrap.<name>`) using `saveState`/`restoreState` + `genExternCall` to handle sret/ABI coercion. The coroutine calls the wrapper (which returns the Promise-internal type) instead of the extern directly. | 5c | ~~Medium~~ Resolved |
| B0045 | Blocking select (no default) silently skips in batch tests. When `inCoroutine=false` (batch test functions), the SWN waiter-list parking path is not emitted — control falls through to the merge block immediately without selecting any case. Only affects batch test functions; snapshot tests and normal `main()` are coroutines and work correctly. Workaround: use `select` with a `default` branch, or use snapshot tests for blocking select scenarios. | 5c | Low |
| B0110 | Select send case wake path may write to full/closed channel. Between wake (receiver drains one slot) and select goroutine resuming, another sender could fill the slot. Recv case is safe (`execRecv` checks emptiness). Requires Go-style direct-transfer to fix properly. | 5c | Low |
...
| B0020 | Extern ABI for generic types | 8f | Low |
| B0021 | Non-instance field placements: mixed `value`+instance, `variant`/`type` fields, `global`/`mono` data placement | 8c | Low |
...
| B0022 | `yield*` delegate (forward all values from sub-iterator) | Generators | Medium |
| B0023 | Failable generators (`stream[T]!` with error propagation through yield) | Generators | Low |
| B0024 | Stored generator values (first-class generator variables outside for-in) | Generators | Low |
| B0025 | Generator closures (capturing lambdas as generators) | Generators | Low |
...
| B0026 | Devirtualization optimization (direct call when concrete type known) | 8L | Low |
| ~~B0027~~ | ~~PAL `EmitFileStatSize` depends on `lookupFunc` for `pal_file_open`/`pal_file_close` — will panic if emission order in `declareIntrinsics()` changes~~ — **Fixed.** Added `getOrDeclareFunc` helper in `pal.go` that looks up an existing function or declares it if not found. `EmitFileStatSize` on both POSIX and Windows now uses `getOrDeclareFunc` for `open`/`close`/`lseek` (POSIX) and `_open`/`_close`/`_lseeki64` (Windows), making them order-independent. | D | ~~Low~~ Resolved |
| ~~B0028~~ | ~~`os.execute` reads stdout/stderr sequentially — deadlock if child writes >64KB to stderr while stdout pipe is not drained~~ — **Fixed.** Decomposed `pal_execute` into `pal_spawn` + `pal_read_pipe` + `pal_wait_pid`. `execute()` in os.pr reads both pipes concurrently using goroutines (`go _os_read_pipe(stderr_fd)` for stderr, `_os_read_pipe(stdout_fd)` for stdout). Each pipe read releases the scheduler P via enter/exit_syscall. | E | ~~Medium~~ Resolved |
...
| B0030 | Optional user-defined type in constructor — `Coord? location` field causes codegen panic in `genConstructorCallMono`: `store operands not compatible: src={i1, {i8*, i8*}}; dst={i1, i8*}*`. The optional wrapping of a user type produces `{i1, {i8*, i8*}}` but the constructor field expects `{i1, i8*}`. Workaround: none — optional fields of user-defined types in constructors don't work. Blocks `T? field` in `serializable` types where T is a user type. | Codegen | Medium |
| ~~B0031~~ | ~~`map[K, Enum]` monomorphization~~ — **Fixed.** Root cause: `llvmType`, `llvmTypeForEnumFieldFromPromise`, and `instanceFieldLLVMType` had no `*types.Enum` case — enums used as fields in user types or enum variant data returned `i8*` instead of the correct internal type (`i32` for fieldless, `{i32, [N x i8]}` for data enums). Fix: added `enumInternalTypeForField` helper that looks up pre-computed enum layouts (named struct types) and falls back to `computeEnumInternalType` from the definition. Added `ptrSize` + enum layout maps to `llvmTypeForEnumFieldFromPromise` and `instanceFieldLLVMType`. Added dependency-ordered layout computation in `computeEnumLayouts` (topological sort for forward-declared enums) and two-pass `computeMonoLayouts` (enums first, then types). Also fixed a latent data area sizing bug: enum variant data size was computed as sum of individual field sizes (ignoring inter-field alignment padding); now uses `llvmTypeSizeWithPtr(structType, ptrSize)`. Also fixed pre-existing optional field member assignment: `h.field = value` where field is `T?` now auto-wraps the value. 53 tests in `enum_field_test.pr` + `enum_field_advanced_test.pr`. | Codegen | ~~Medium~~ Resolved |
| ~~B0044~~ | ~~`for k, v in map[K, Enum]` crashes cross-module~~ — **Fixed.** Root cause: enum layouts for catalog module enums (e.g. `JsonValue` from json) were not available when the std module's `Map[K, V]` methods were monomorphized. The `Slot[K, V]` variant data struct used anonymous LLVM types for the enum fields, but `resolveType` later found the named type from the module's layout — LLVM type identity mismatch. Fix: pre-compute enum layouts for ALL imported modules at the start of `compile()`, before any mono layout computation. Also fixed `resolveType` fallback to use `computeEnumInternalType` instead of `i32` for enums not in layouts. 13 same-file tests in `enum_map_iteration_test.pr` + 7 cross-module tests in `enum_map_cross_module_test.pr`. | Codegen | ~~High~~ Resolved |
...
| # | Item | Origin |
|---|------|--------|
| B0038 | `inline`, `packed`, `align`, `extern`, `serializable`, `public`, `unsafe` processing | 7 |

### Parser / Codegen Bugs

| # | Item | Origin | Priority |
|---|------|--------|----------|
| B0001 | `match` as expression in return context (`return match v { ... }`) does not parse — the parser rejects `match` as an expression after `return`. Workaround: use if/else chains or assign match result to a variable first. | Parser | Medium |
| B0002 | `match` arms with block bodies and commas — `'a' => { stmt; }, 'b' => { stmt; },` does not parse. The comma after `}` is rejected. Match works for expression arms (`'a' => value,`) but block-body arms for side-effecting code (writes, mutations) cannot use `match`. Workaround: use if/else chains for imperative dispatch. | Parser | Medium |
| ~~B0003~~ | ~~`\{` escape codegen panic~~ — **Done.** Root cause: empty interpolation `{}` produced a `StringInterp` with nil `Expr`; sema silently skipped it, codegen panicked in `convertToString` with nil type. Fix: sema now reports "empty interpolation '{}' in string literal" error; codegen skips nil-`Expr` interpolation parts defensively. `\{` escape itself was always correctly lexed as `STRING_ESCAPE`. | Codegen | ~~High~~ Resolved |
| ~~B0004~~ | ~~`while` unwrap binding borrow conflict~~ — **Done.** Call-scoped borrows from condition/init expressions in `while`, `while-unwrap`, `if`, `for-in`, and classic `for` were not expired before checking the body, blocking re-borrowing. Fixed by adding `ExpireCallScoped()` between condition evaluation and body checking in all five control flow handlers. Filed B0103 for the same pattern in `select` (low priority). | Ownership | ~~Medium~~ Resolved |
| B0005 | Stale module cache on std file addition — adding a new `.pr` file to `modules/std/` (e.g., `encode.pr`) shifts string constant numbering in the module IR, but previously cached `.bc` files for other modules referencing those constants are not invalidated. Results in linker errors (`undefined symbol: .str.N`). Workaround: `promise clean` to clear the cache. The cache key hashes module IR text, but the old cache entries from before the std change persist and are matched by stale callers. | Cache | Medium |
...
| ~~No narrowing after diverging `is absent` check~~ — **Done.** `if x is absent { return; }` now narrows `x` to `T` after the if. Implemented via `preDetectIfNarrowing` detecting `is absent` as negated narrowing, `pendingNarrowings` in `checkBlock` opening a scope shadow, and `PostNarrow` flag for codegen to extract the inner value at the merge point. Also works for `if !x { return; }`. | Sema | ~~High~~ Resolved |
...
| ~~T0002~~ | ~~Enum `` `serializable `` codegen~~ — **Done.** Simple enums encode as string; data enums use tagged object format `{"type":"Variant",...fields...}`. Decoder requires `"type"` as first key (discriminator-first constraint). Also enabled `factory` annotation on enum methods. 52 tests in `enum_serializable_test.pr`. | ~~B0015~~ (resolved) | ~~Medium~~ Resolved |
| ~~T0008~~ | ~~Data enum `` `serializable `` codegen~~ — **Done.** Implemented as part of T0002. Tagged object format with discriminator-first decode. | ~~T0002~~ (resolved) | ~~Medium~~ Resolved |
| ~~T0003~~ | ~~`` `serializable(tag: "kind") `` parameter~~ — **Done.** Custom discriminator field name for data enum serialization (default: `"type"`). Tag stored on `Enum` type, passed to encode/decode synthesis. 7 new tests (encode, decode, fieldless, roundtrip, wrong/missing discriminator). Filed B0133 (deferred) for tag-field name collision validation. | ~~T0008~~ (resolved) | ~~Low~~ Resolved |
| ~~T0004~~ | ~~`` `flatten `` field annotation~~ — **Done.** Inlines nested type fields into parent during encode/decode. Validates type constraints, annotation conflicts (`key`, `include_none`), and wire name collisions. Extracted `encodeFieldStmts` helper and `decodeField` struct for unified key-match chain. 9 e2e tests + 8 sema tests. | — | ~~Medium~~ Resolved |
| ~~T0005~~ | ~~`json.encode_string[T]` / `json.decode_string[T]` with `serializable` user types~~ — **Done.** Fixed three compiler bugs: (1) codegen routed module-qualified generic calls (`json.func[T]()`) to method dispatch instead of function dispatch, (2) sema didn't record `FuncInstance` for module-qualified generic calls, (3) `compileModule` didn't declare/define enum methods. `encode_string[T]`/`decode_string[T]`/`encode_string_pretty[T]` now work with all `serializable` types. 11 tests. | — | ~~Medium~~ Resolved |
| ~~T0006~~ | ~~Enum methods for ergonomic `JsonValue` API~~ — **Done.** Added 16 methods to `JsonValue` enum: type-check getters (`is_null`/`is_bool`/`is_number`/`is_string`/`is_array`/`is_object`), value extractors (`as_bool`/`as_number`/`as_string`/`as_array`/`as_object`), convenience access (`get(key)`/`at(index)`), encoding (`encode`/`format`/`format_pretty`). Free functions (`encode_value`/`format_value`/`format_value_pretty`) delegate to methods. Also fixed module enum method compilation (added `declareModuleEnumMethods`/`defineModuleEnumMethods`). 35 tests. | ~~B0015~~ (resolved) | ~~Low~~ Resolved |
| T0007 | Additional format modules — `modules/toml/`, `modules/yaml/`, `modules/msgpack/`. Each implements `Encoder`/`Decoder`. All `` `serializable `` types work automatically. | — | Low |
...
Wired into `compileAndLinkSeparate()` in `main.go`: lookup → cache hit skips `opt+llc` → cache miss compiles and saves. A JSON `.o.meta` sidecar (`CacheMeta` struct) is stored alongside each `.o` file for incremental dependency tracking and debugging.
...
Each generic type/function instantiation gets its own cached compiled artifact (`.o` in the build cache), enabling fine-grained reuse when only some instances change. On the LTO path (Linux default) the file contains LLVM bitcode; on other paths it is a native object file. Either way it is stored with the `.o` extension — `ld.lld` identifies the format by magic bytes, not extension.
...
**Cache key** (`module.InstanceCacheKey`): `FNV-128a("instance\nprefix:<irPrefix>\nmono:<monoName>\ndecl:<typeDeclHash>\ncompiler:<compilerHash>\ntarget:<target>\n")`. The `monoName` encodes the concrete type args (`Box[int]`, `Map[string, Vector[int]]`). The `typeDeclHash` covers the entire generic type definition. The `irPrefix` scopes it to the owning module.

**Pre-codegen lookup** (`lookupCachedInstances` in `main.go`): Before calling `codegen.Compile`, collects all mono instance names from `sema.Info`, builds their cache metadata via `buildInstCacheMetas`, and checks the build cache (`module.LookupBuildCache`). Only active when modules are present and not using the clang pipeline. Returns `map[string]bool` of pre-cached instance names.
...
**Post-codegen pre-cached loop** (`compileAndLinkSeparate` in `main.go`): Before the goroutine loop that compiles fresh instance IRs, a synchronous loop iterates all instance cache keys. For instances missing from `InstanceIRs()` (pre-cached), it calls `module.LookupBuildCache` and appends the cached `.o` path directly to `instObjs`. If a cache file has vanished concurrently (e.g., `promise clean`), the linker reports an undefined symbol — correct and safe.
...
**Files:** `compiler/internal/sema/typehash.go` (HashTypeDecl/HashEnumDecl), `compiler/internal/sema/info.go` (DeclHashes field), `compiler/internal/module/cache.go` (InstanceCacheKey, CacheMeta, BuildCacheMetaPath, ReadBuildCacheMeta, SaveBuildCache), `compiler/internal/codegen/compiler.go` (CompileWithCache, instanceOwnedFuncs, cachedInstances), `compiler/internal/codegen/separate.go` (InstanceIRs, saveAndStripNonOwnedInst), `compiler/cmd/promise/main.go` (lookupCachedInstances, buildInstCacheMetas, pre-cached loop in compileAndLinkSeparate)
...
**Files:** `cmd/promise/main.go` (`cachedStdHash`, `computeTestFileCacheKey`, `executeE2EBinary`), `internal/module/cache.go` (`HashFile`, `HashDir`, `CacheMeta`, `SaveTestBinaryMeta`/`LoadTestBinaryMeta`), `cmd/promise/stress.go` (cache lookup in `compileTargets`)
...
- **Unified sidecar format**: All cached artifacts (module `.o`, instance `.o`, test `.bin`) use a single `CacheMeta` JSON sidecar with a `kind` discriminator (`"llvm_module"`, `"instance"`, `"test_binary"`), plus common fields (`name`, `cache_key`, `created_at`) and kind-specific fields (`interface_hash` for modules, `type_decl_hash`/`ir_prefix` for instances, `e2e`/`expected_output`/`exclude_targets`/`tests`/`test_excludes` for binaries). Module/instance sidecars use the `.o.meta` extension; test binary sidecars use `.bin.meta`.
