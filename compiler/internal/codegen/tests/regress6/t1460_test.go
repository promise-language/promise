...
func TestT1460ViewAdapterDropsHeapStringDefault(t *testing.T) {
	ir := codegentest.GenerateIR(t, t1460TaggerIface+`
type HeapParent {
  int v;
...
`)
	adapter := codegentest.ExtractDefine(ir, "HeapParent.tag$view_adapt_as_Tagger")
	if adapter == "" {
		t.Fatalf("no view adapter emitted\n%s", ir)
	}
	codegentest.AssertContains(t, adapter, "@promise_string_concat")
	codegentest.AssertContains(t, adapter, "call void @promise_string_drop")
	codegentest.AssertContains(t, adapter, "adapt.alias.clear")
}
...
`)
	adapter := codegentest.ExtractDefine(ir, "UserDefP.tag$view_adapt_as_Tagger")
	if adapter == "" {
		t.Fatalf("no view adapter emitted\n%s", ir)
	}
	// The constructed default is freed on both the normal and the unwind path.
	codegentest.AssertContains(t, adapter, "call void @pal_free")
...
`)
	adapter := codegentest.ExtractDefine(ir, "TupP.tag$view_adapt_as_Tagger")
	if adapter == "" {
		t.Fatalf("no view adapter emitted\n%s", ir)
	}
	codegentest.AssertContains(t, adapter, "tuptmp.drop")
	codegentest.AssertContains(t, adapter, "call void @promise_string_drop")
}
...
`)
	adapter := codegentest.ExtractDefine(ir, "ArrP.tag$view_adapt_as_Tagger")
	if adapter == "" {
		t.Fatalf("no view adapter emitted\n%s", ir)
	}
	codegentest.AssertContains(t, adapter, "arrtmp.drop")
	codegentest.AssertContains(t, adapter, "call void @promise_string_drop")
}
...
`)
	adapter := codegentest.ExtractDefine(ir, "GenP[int].tag$view_adapt_as_Tagger")
	if adapter == "" {
...
`)
	adapter := codegentest.ExtractDefine(ir, "MutRefP.tag$view_adapt_as_Tagger")
	if adapter == "" {
		t.Fatalf("no view adapter emitted\n%s", ir)
	}
	// The concrete method takes i8**; the adapter must forward an alloca, not the
	// i8* the concat produced.
	codegentest.AssertContains(t, adapter, "alloca i8*")
	codegentest.AssertContains(t, adapter, "@MutRefP.tag(i8* %this, i8** ")
...
`)
	adapter := codegentest.ExtractDefine(ir, "VoidP.run$view_adapt_as_Runner")
	if adapter == "" {
		t.Fatalf("no view adapter emitted\n%s", ir)
	}
	codegentest.AssertContains(t, adapter, "call void @promise_string_drop")
	codegentest.AssertNotContains(t, adapter, "adapt.alias.clear")
}
...
`)
	adapter := codegentest.ExtractDefine(ir, "FailVoidP.run$view_adapt_as_FailRunner")
	if adapter == "" {
		t.Fatalf("no view adapter emitted\n%s", ir)
	}
	codegentest.AssertContains(t, adapter, "call void @promise_string_drop")
...
`)
	adapter := codegentest.ExtractDefine(ir, "IntP.count$view_adapt_as_Counter")
	if adapter == "" {
		t.Fatalf("no view adapter emitted\n%s", ir)
	}
	codegentest.AssertContains(t, adapter, "call void @promise_string_drop")
...
`)
	adapter := codegentest.ExtractDefine(ir, "JoinP.join$view_adapt_as_Joiner")
	if adapter == "" {
		t.Fatalf("no view adapter emitted\n%s", ir)
	}
	// The interface's `head` arrives as %p0 and is forwarded straight through.
	codegentest.AssertContains(t, adapter, "i8* %p0")
	codegentest.AssertContains(t, adapter, "@JoinP.join(i8* %this, i8* %p0")
...
`)
	adapter := codegentest.ExtractDefine(ir, "CoroP.tag$view_adapt_as_Tagger2")
	if adapter == "" {
		t.Fatalf("no view adapter emitted\n%s", ir)
	}
	...
	codegentest.AssertNotContains(t, adapter, "llvm.coro")
	codegentest.AssertNotContains(t, adapter, "coro.suspend")
	codegentest.AssertNotContains(t, adapter, "coro.cleanup")
}
...
`)
	...
	dropper := codegentest.ExtractDefine(ir, "EnvP.call$view_adapt_as_EnvCaller")
	if dropper == "" {
		t.Fatalf("no view adapter emitted for EnvP.call\n%s", ir)
	}
	codegentest.AssertContains(t, dropper, "env.tmp.drop")
	codegentest.AssertNotContains(t, dropper, "adapt.alias.clear")
	...
	aliaser := codegentest.ExtractDefine(ir, "EnvAliasP.make$view_adapt_as_EnvMaker")
	if aliaser == "" {
		t.Fatalf("no view adapter emitted for EnvAliasP.make\n%s", ir)
	}
	codegentest.AssertContains(t, aliaser, "env.tmp.drop")
	codegentest.AssertContains(t, aliaser, "adapt.alias.clear")
}
...
`)
	adapter := codegentest.ExtractDefine(ir, "ChanP.tag$view_adapt_as_Tagger")
	if adapter == "" {
		t.Fatalf("no view adapter emitted\n%s", ir)
	}
	codegentest.AssertContains(t, adapter, "@promise_channel_new")
	codegentest.AssertContains(t, adapter, `@"Channel[int].drop"`)
...
`)
	adapter := codegentest.ExtractDefine(ir, "MoveP.tag$view_adapt_as_Tagger")
	if adapter == "" {
		t.Fatalf("no view adapter emitted\n%s", ir)
	}
	// The consumed param is passed as `none` and the adapter drops nothing.
	codegentest.AssertContains(t, adapter, "zeroinitializer")
	codegentest.AssertNotContains(t, adapter, "promise_string_drop")
	codegentest.AssertNotContains(t, adapter, "adapt.alias.clear")
}
...
`)
	adapter := codegentest.ExtractDefine(ir, "LitP.tag$view_adapt_as_Tagger")
	if adapter == "" {
		t.Fatalf("no view adapter emitted\n%s", ir)
	}
	codegentest.AssertNotContains(t, adapter, "promise_string_drop")
	codegentest.AssertNotContains(t, adapter, "adapt.alias.clear")
	codegentest.AssertNotContains(t, adapter, "alloca")
}
