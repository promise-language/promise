...
Promise is a statically-typed programming language with Dart-inspired syntax and Rust-inspired ownership semantics. The compiler is a single Go binary (`promise`) that uses ANTLR4 for parsing and targets LLVM IR. Two build modes control the backend pipeline: **debug** (default for `build`/`run`/`exec`/`test`) compiles via `opt -O1` → `.bc` → `llc` → `.o` → linker with `--gc-sections` (no LTO); **release** (`--release`) compiles via `opt -O1` → `.bc` → linker with `--lto-O1` (LTO for cross-module inlining and DCE). On Windows, both modes use `opt` → `llc` → `lld-link` (LTO not wired up for MSVC). On WASM release, `--lto-O2` is used. Clang fallback on other platforms.
...
**Debug mode** (default): `opt -O1` → `.bc` → `llc` → `.o` → linker with `--gc-sections` (no LTO). **Release mode** (`--release`): `opt -O1` → `.bc` → linker with `--lto-O1` (`--lto-O2` for WASM). LTO performs whole-program inlining and DCE across all modules at link time. On Windows: `opt -O1` → `llc -filetype=obj` → `lld-link` (LTO not yet wired up for MSVC, both modes same). On other platforms (or `PROMISE_USE_CLANG=1`): `clang -O1`. Build mode is included in all cache keys (module, instance, test binary) to prevent cross-contamination. Requires LLVM 22+. Release builds (`bin/build --release`) embed gzip-compressed LLVM tools in the binary (~61-71MB), extracted lazily to `~/.promise/cache/llvm/<platform>/`. Platform-specific embed files (`llvm_linux_amd64.go`, `llvm_darwin_arm64.go`, `llvm_darwin_amd64.go`) select the correct tools. On macOS, extracted Mach-O binaries are patched with `install_name_tool` (rpath + dylib references) and re-signed with `codesign`.
...
- `emitMonoVtableGlobals(instances)` — creates immutable `[N x i8*]` vtable globals (`promise_vtable_Box__int`). Called at two points in the overall compile sequence: first in the main codegen phase **before** `compileModules()` (module-owned functions not yet declared → some slots are `null`), then again **inside** each `declareModulePhase()` (see *Module compilation order* below). The second call **updates** the existing global's `Init` field with newly available function pointers rather than skipping — this is the critical fix for module-owned generic types.
- `emitMonoTypeInfoGlobals(instances)` — creates immutable typeinfo globals (`promise_typeinfo_Box__int`) with layout `{i8* vtable_ptr, i8* drop_fn_ptr, i8* clone_fn_ptr, i32 typeID, i32 numParents, [N x i32] parentIDs}`. `drop_fn_ptr` (B0226) points to the type's drop function (or null), enabling runtime drop dispatch for untyped error catches. `clone_fn_ptr` (T0387) points to a synthesized per-type clone function (or null), enabling runtime clone dispatch in `dupHeapValue` so polymorphic vector slicing produces correct concrete-subtype copies. TypeID is assigned from `c.nextTypeID++`. For value types, also emits a global RTTI instance (`promise_rtti_Box__int`).
...
**Module compilation order** (`codegen/compiler_module.go`): `compileModules()` walks the imported modules twice in topological order — `declareModulePhase()` for every module (layouts, method/func stubs, vtable + typeinfo globals), then `defineModulePhase()` for every module (bodies). The sweeps must be separate: a std generic monomorphized for a type declared in a *later* module (`sort(Score[])`, `Set[Key]`, `"{item}"`, a synthesized drop) dispatches to that type's method stub from inside a std body, and a strictly per-module declare-then-define order has not created it yet — codegen panics with an undeclared-method/getter/operator miss (T1458). The main file is declared before `compileModules()` and defined after it, so main code is unaffected either way.

**Module separation + caching**: After codegen, `SplitModuleIRs()` assigns functions to module IRs using `moduleOwnedFuncs[name] = moduleName`, registered during the declare phase when `c.compilingModule != ""`. Note that synthesized structural default method stubs are NOT registered (they always stay in the main IR). Module IRs are compiled to `.bc` (LTO) or `.o` and cached. The cache key includes a FNV-128a hash of the **module IR text** (not just source hash), making it sensitive to which mono instances are present — two user programs importing the same module but using different generic instances get separate cache entries. String constants use `LinkagePrivate` so each split `.bc` (module, instance) contains its own copy — no cross-module extern references for strings. Per-module names (`@.str.__mod_std.N`, counter reset to 0 at each module's `defineModulePhase()` start) remain stable within a module compilation. LTO deduplicates identical private globals at link time.
...
| `net` | `modules/net/net.pr` | TCP networking: `TcpListener` (bind/accept/close), `TcpStream` (connect/read/write/close/shutdown — `connect` takes a host name or an IPv4/IPv6 literal), `resolve` (A/AAAA lookup via the platform resolver, behind the scheduler's syscall handoff rather than the reactor), `NetError`, `ResolveError`/`ResolveErrorKind`, reactor-based non-blocking I/O; per-operation deadlines (`set_read_deadline`/`set_write_deadline`/`set_accept_deadline`, `connect` deadline argument) and `CancelHandle` cancellation |
| `http` | `modules/http/http.pr` | HTTP/1.1 client + server over http:// and https://: `Request`/`Response`, `http_get`/`http_post`/`http_post_json`; `Client` (redirect following, keep-alive connection pooling, gzip, `set_tls_config` for custom CAs / mutual TLS); `Server.bind` (HTTP) / `Server.bind_tls` (HTTPS) with `Handler` — one goroutine per connection, keep-alive, bounded concurrency (`max_connections`/`max_keep_alive_requests`), draining graceful shutdown; `Handler.handle` takes a shared `this`, so handler state must be immutable or `Mutex`-guarded. Importing `http` links a TLS backend even for plaintext-only programs (linux-amd64: 13.4 MB debug / 19.0 MB release, vs 0.63 MB for `net` alone) |
| `tls` | `modules/tls/tls.pr` | TLS 1.2/1.3 over TCP: `TlsConfig` (`create`/`insecure`, custom CA, client certificate, min version), `TlsServerConfig` (certificate chain + key, shared across connections), `TlsStream` (satisfies `Reader`/`Writer`; `connect` for clients, `accept` to upgrade an already-accepted `net.TcpStream`, `close_gracefully` for a non-racing close), `TlsListener` (bind/accept — handshakes inline, `local_port`), `TlsError`/`TlsErrorKind`. Backends: Linux links the vendored musl-static OpenSSL (T1596), macOS uses Secure Transport (T1599), Windows uses SChannel (T1598); WASM raises `TlsError(kind: unsupported)` |
...
- **Never use timing (`sleep`) for test synchronization** (T1632). Tests must synchronize **deterministically** — over a channel, a ready/done handshake, or by awaiting a `Task` handle — never by sleeping and assuming a concurrent event has happened by the time the sleep returns. A `sleep` used as a stand-in for a happens-before edge (`sleep(100ms) // give the server time to start`, `sleep(30ms) // let the goroutine finish before the leak snapshot`) is inherently load-sensitive: the window that passes on an idle laptop collapses under a saturated CI runner, producing intermittent timeouts, leaks, and segfaults that can never be fixed by lengthening the sleep — only by removing the timing dependency. This is **not** a ban on testing timing *behavior*: a test of `sleep()`, `Duration`, `Instant`, or a timeout feature legitimately measures time. The rule is specific — do not use a sleep to order two concurrent operations you could order explicitly. Prefer a bind→signal-port-over-channel handshake to "server is up", and a completion channel to "the goroutine finished", so drop/leak accounting never races a clock. **A fire-and-forget goroutine needs no barrier at all**: the batch harness drains every outstanding goroutine back to the test's own pre-test baseline before it reads `alloc_count` (T1639), so the drain — not the test — is what the leak check waits on. Joining also makes a goroutine's assertions count, since a failing `assert()` inside an unjoined `go` block is recovered and discarded (T2014). See `docs/code-style.md` §"Test synchronization" for the join template and the `// sleep-ok:` marker, and `docs/org/engineering-guide.md` §"Time is not a coordinate" for the binding rule. A pre-commit guard rejects any unannotated `sleep()` in a test file.

- **Use full English words in APIs — with approved abbreviations.** All public names in the standard library and language APIs must use complete, unabbreviated words (e.g., `print_line` not `println`, `make_directory` not `mkdir`, `concatenate` not `concat`, `execute` not `exec`). **Exception:** An explicit abbreviation dictionary (see [Naming Conventions](docs/language-design.md#naming-conventions) in `docs/language-design.md`) defines universally preferred short forms — when a mapping exists (e.g., `dir` ← `directory`, `env` ← `environment`, `id` ← `identifier`, `arg` ← `argument`, `attr` ← `attribute`, `ch` ← `character`, `src` ← `source`, `init` ← `initialize`, `len` ← `length`, `millis` ← `milliseconds`, `config` ← `configuration`, `info` ← `information`, `func` ← `function`, `hex` ← `hexadecimal`, `dest` ← `destination`, `pos` ← `position`, `prev` ← `previous`, `var` ← `variable`, `min`/`max`/`abs`), the abbreviation is mandatory. This optimizes for AI-agent readability while avoiding identifiers that look silly when fully expanded. **Proper names are used verbatim** — technologies, formats, protocols, algorithms and standards keep their own spelling even when it looks like an abbreviation and is absent from the dictionary (`base64`, `utf8`, `json`, `sha256`, `url`); only the casing follows Promise convention (`decode_base64`, `JsonEncoder`, not `JSONEncoder`). The abbreviation rule covers shortening English words, not renaming names.
