...
// returns the payload type they are generated from. Uses the mono name so
// Channel[int] and Channel[string] get distinct thunks calling their own drop.
//
// T1885: the active substitution is applied first. Without it a box emitted inside
// a monomorphized body keyed on the UNBOUND instance (Vector[T]) — and since the
// box's drop_fn and clone_fn are generated from the element type, Vector[int] and
// Vector[string] would then have shared one wrong pair.
func (c *Compiler) containerBoxKey(fromNamed *types.Named, fromType types.Type) (string, types.Type) {
	if c.typeSubst != nil {
	...
	if fromNamed != nil {
		return fromNamed.Obj().Name(), fromType
	}
	return "", fromType
}
...
// { i8* typeinfo, i8* handle }), loads the handle from field 1, drops it, then
// frees the box. This is the drop_fn carried by the box's typeinfo header, which is
// what makes an opaque handle droppable through __promise_structural_drop at every
// RTTI drop site (T1887). Returns nil when the concrete type cannot be named.
//
// The payload drop is the ordinary field/variant drop walk (emitVariantFieldDrop) —
// the same one a struct field or an enum variant of this type gets — so a Vector's
// elements are dropped before its buffer, a Channel releases its buffer and lock,
// and a Ref's strong count is decremented exactly as they are anywhere else. T1885:
// resolving the drop by name (resolveTypeInfoDropFn) yielded the bare `Vector.drop`
// for a Vector[string], which freed the buffer and leaked every element. A static
// (.rodata) vector reaching here is safe: its elements are compile-time constants,
// so no element drop loop is emitted, and Vector.drop itself skips the free when
// bit 63 is set.
func (c *Compiler) getContainerBoxDrop(fromNamed *types.Named, fromType types.Type) *ir.Func {
	...
	boxType := irtypes.NewStruct(irtypes.I8Ptr, irtypes.I8Ptr)
	typed := c.block.NewBitCast(boxParam, irtypes.NewPointer(boxType))
	hField := c.block.NewGetElementPtr(boxType, typed,
		constant.NewInt(irtypes.I32, 0), constant.NewInt(irtypes.I32, 1))
	handle := c.block.NewLoad(irtypes.I8Ptr, hField)
	c.emitNullGuardedVariantDrop(handle, func() {
		c.emitVariantFieldDrop(handle, payloadType)
	})
	c.block.NewCall(c.palFree, boxParam)
	c.block.NewRet(nil)
	return fn
}

// getContainerBoxClone returns @__promise_container_box_clone$<T>, emitting it once
// per concrete: a fresh box carrying the same typeinfo header and an independently
// owned copy of the payload, so __promise_structural_clone deep-copies a boxed
// container instead of aliasing it (which the structural-aware element drop would
// then double-free). T1885, mirroring getStringBoxClone.
//
// Returns nil for a handle that has no dup semantics (Mutex, MutexGuard, Task —
// T1113): a structural clone/slice of one must not fabricate a second owner of the
// same handle, so its box keeps a null clone_fn (T1887).
func (c *Compiler) getContainerBoxClone(key string, payloadType types.Type) *ir.Func {
	if !nativeDupSupported(payloadType) {
		return nil
	}
	if c.containerBoxClones == nil {
		c.containerBoxClones = map[string]*ir.Func{}
	}
	if fn, ok := c.containerBoxClones[key]; ok {
		return fn
	}
	boxParam := ir.NewParam("box", irtypes.I8Ptr)
	fn := c.module.NewFunc(fmt.Sprintf("__promise_container_box_clone$%s", key), irtypes.I8Ptr, boxParam)
	c.containerBoxClones[key] = fn
	...
	oldPayloadField := c.block.NewGetElementPtr(boxType, oldTyped,
		constant.NewInt(irtypes.I32, 0), constant.NewInt(irtypes.I32, 1))
	oldPayload := c.block.NewLoad(irtypes.I8Ptr, oldPayloadField)
	newPayloadField := c.block.NewGetElementPtr(boxType, newTyped,
		constant.NewInt(irtypes.I32, 0), constant.NewInt(irtypes.I32, 1))
	// Seed the slot: emitVariantFieldDup only writes it for the types it can dup,
	// and this thunk is only ever built for those (nativeDupSupported above).
	c.block.NewStore(oldPayload, newPayloadField)
	c.emitVariantFieldDup(oldPayload, newPayloadField, payloadType)
...
// per-type thunk above; clone_fn (field 2) is the per-type clone thunk for a
// duplicable payload and null otherwise. Returns nil when the type has no drop.
