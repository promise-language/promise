...
func TestT0512_NestedIfUnwrapBorrowedMatchBindingNoInnerDrop(t *testing.T) {
	...
	`)
	body := codegentest.ExtractFunction(ir, "__user._drive")
	if body == "" {
	...
	enumDrop := codegentest.ExtractFunction(ir, "_W.drop")
	if enumDrop == "" {
...
func TestT0512_NestedWhileUnwrapBorrowedMatchBindingNoInnerDrop(t *testing.T) {
	...
	`)
	body := codegentest.ExtractFunction(ir, "__user._drivew")
	if body == "" {
...
func TestT0512_OwnedNestedUnwrapStillRegistersDrop(t *testing.T) {
	ir := codegentest.GenerateIR(t, `
		...
		main() { _owned(); }
	`)
	body := codegentest.ExtractFunction(ir, "__user._owned")
	if body == "" {
