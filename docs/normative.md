...
than restated ([What a gap is filed as](#what-a-gap-is-filed-as) there).
...
## Granularity one item per document
...
This is what makes [Reconciliation](org/normative.md#reconciliation)'s invariant hold at a bounded item count.
...
## When the pass runs forward only

Every ratification and amendment runs the pass over its own delta, as its own change
([org/normative.md](org/normative.md), under [Reconciliation](org/normative.md#reconciliation)). That is the whole steady-state rule, and nothing periodic
sweeps behind it.
...
## What a gap is filed as

The three kinds of gap — unbuilt, divergent, unspecified — are named in
[org/normative.md](org/normative.md), under [Reconciliation](org/normative.md#reconciliation), and the tag facets in [tags.md](tags.md). This project
files them as:
...
The table governs a row promoted out of a document's item ([Granularity one item per document](#granularity-one-item-per-document)). The document's own item is always a
...
[Reconciliation](org/normative.md#reconciliation)'s invariant is upheld by review, exactly as its [What is enforced mechanically](org/normative.md#what-is-enforced-mechanically) says.
...
One surface is the exception, and it is project-local: [annotations.md](annotations.md), under [Index](annotations.md#index)'s
...
[A specification states the end state](org/normative.md#a-specification-states-the-end-state)'s ban on inline markers: the exception has a home, and the
