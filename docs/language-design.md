...
**Evaluation order of compound assignment through index and slice access is fixed**: `target → key/bounds → RHS → read → operator → write`. The target and key (or slice bounds) are evaluated first, then the right-hand side is evaluated *before* the getter read, and the target is evaluated exactly once — the getter (`[]`/`[:]`) and setter (`[]=`/`[:]=`) reuse that single evaluation. This is observable only when the RHS side-effects the same target (e.g. `s[a:b] += f()` where `f()` mutates `s`), in which case the read combines the post-RHS value. All index (`[]`), slice (`[:]`), native-container, and field/setter (`a.b += x`) paths follow this one order — the target (receiver) is evaluated first and exactly once, then the RHS, then the getter read. The same single evaluation applies to plain mutation through a place: a native container mutator (`v.push(x)`, `v.pop()`, `v.remove(i)`), an index or slice assignment (`v[i] = x`, `v[a:b] = ...`) and an increment (`v[i]++`) each evaluate their place exactly once, and a reallocated buffer is written back into that same evaluation — so `make().items.push(x)` grows and writes back the field of the one object `make()` returned.
...
- **Override signatures**: overriding is not renaming a slot. A declaration that takes over an inherited slot must **satisfy** the nearest inherited declaration of that slot under the relaxed matching rules of [Structural Interface Satisfaction](#structural-interface-satisfaction) — extra defaulted parameters, non-failable for failable, `T` for `T?`, covariant return, and (as for any explicit `is`) an exactly matching receiver borrow kind. Anything else is a declaration error naming both signatures. *Nearest*, not original: where `R is Q is P` and `Q` legally relaxed `P`'s `close!(~this)` to `close(~this)`, `R` is measured against `Q`, because `Q` is the shape a `Q`-typed call site reads. Constructors (`new`) and `` _validate! `` are exempt — neither shares a slot with its parent's, and both have their own inheritance rules (see [Constructors](#constructors) and [Validation](#validation)).
...
For `Copy` types (primitives, `char`, `bool`, pure value types) every mode is a by-value copy, so the distinction is irrelevant; it matters only for move types (`string`, collections, heap user types). A `move` parameter is the only way to consume an argument; a borrow parameter may read (and, if `~`, modify) the value but may **not** move it out — into a field, another `move` callee, or a capture. This prevents double-free bugs where both the caller and a transitive consumer would drop the same allocation.

**Returning a borrow parameter by value is allowed, and it is safe.** `id(string s) string { return s; }` compiles: the caller still owns its argument, and the callee hands back a value the caller will also own. This is the natural way to write "return the input unchanged on the fast path" — the `strings`, `path`, and `os` modules all rely on it — so rather than reject the shape, the compiler makes it sound by **duplicating on alias**: at the call site it compares the returned pointer against the pointers it passed in, and deep-clones the result only when they actually match, leaving the caller with two independent values. A callee that returns a *freshly built* value pays nothing.

The duplication is a copy the source does not spell out, which is why it is confined to this one shape. It changes no ownership the reader can observe — the caller keeps the argument it already owned and owns the result, exactly as the signature says — and the alternatives are worse: declaring such functions `move` would consume arguments callers still need, and requiring `.clone()` at every call site would pay for the copy even when the callee returns something fresh.
