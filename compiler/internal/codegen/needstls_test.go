...
func TestNeedsTLS(t *testing.T) {
	cases := []struct {
		...
		{"no tls externs", []*ExternFunc{{CName: "promise_string_len"}, {CName: "pal_write"}}, false},
		{"one tls extern", []*ExternFunc{{CName: "promise_string_len"}, {CName: "promise_tls_client_new"}}, true},
		{"only tls extern", []*ExternFunc{{CName: "promise_tls_handshake"}}, true},
		{"nil entry is skipped", []*ExternFunc{nil, {CName: "promise_tls_read"}}, true},
		{"prefix must be exact", []*ExternFunc{{CName: "xpromise_tls_read"}}, false},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			r := &CompileResult{Externs: tc.externs}
