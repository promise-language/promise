...
func TestT1200_WasmNonCoroutineChannelWaitPumpsCoopStep(t *testing.T) {
	ir := codegentest.GenerateIRForTarget(t, t1200ChannelSrc, "wasm32-wasi")

	worker := codegentest.FindDefinedFunc(ir, "@__user.worker(")
	if worker == "" {
...
func TestT1200_WasmNonCoroutineReceiveWaitPumpsCoopStep(t *testing.T) {
	...
	`
	ir := codegentest.GenerateIRForTarget(t, src, "wasm32-wasi")

	consumer := codegentest.FindDefinedFunc(ir, "@__user.consumer(")
	if consumer == "" {
...
func TestT1200_NativeChannelWaitUnchanged(t *testing.T) {
	ir := codegentest.GenerateIRForTarget(t, t1200ChannelSrc, "x86_64-unknown-linux-gnu")
