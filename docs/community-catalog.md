...
It builds directly on the epoch-aware resolution + verification engine (epoch-tag walk-back, the empirical "compiles + 100% of `` `test `` functions pass" gate, and the [When a Module Has No Compatible Version](module-system.md#when-a-module-has-no-compatible-version) no-compatible-version error) — that engine is the single source of truth for "compatible"; the community tier only adds a registry, its CI, and the name-tier integration.

---

## Why a separate living repo

The embedded first-party catalog (`catalog.toml`) is **frozen in the compiler binary** — one per epoch. That is correct for modules the language team owns and updates atomically ([Breaking Changes Between Epochs](module-system.md#breaking-changes-between-epochs)), but it cannot record a verdict that is only discovered *after* an epoch ships.
...
The repo URL is a well-known constant (`https://github.com/promise-community/catalog`), overridable with the `PROMISE_COMMUNITY_CATALOG` environment variable for mirrors and air-gapped environments ([Open Questions](module-system.md#open-questions)).
...
### The per epoch compatibility index
...
A module **absent** from `index/<E>.json` means *no verified version for epoch E* — the [When a Module Has No Compatible Version](module-system.md#when-a-module-has-no-compatible-version) signal.
...
### The published module and epoch grid
...
## Tiers and name resolution

A module's **tier is determined by its URL** — there is no separate flag:

| Tier | URL | Compatibility established by |
|------|-----|------------------------------|
| Embedded / first-party | no URL, or `github.com/promise-language/*` | epoch release gate ([Epoch Guarantees](module-system.md#epoch-guarantees)) — always 100% |
...
`promise package add name` / `use name` resolves names in this order ([Compatibility and the Community Catalog](module-system.md#compatibility-and-the-community-catalog)):

1. `[replace]` (local override, [Local Development Overrides](module-system.md#local-development-overrides))
2. `[require.NAME]` (project alias, [The per epoch verify and tag loop](#the-per-epoch-verify-and-tag-loop))
3. embedded first-party catalog ([The catalog manifest](module-system.md#the-catalog-manifest)) — no pin needed
...
- **verified entry present** → pins that commit in `[require]` and reports it. **No local test run happens** — for community modules the CI index *is* the verdict ([Compatibility and the Community Catalog](module-system.md#compatibility-and-the-community-catalog)).
- **no entry for E** → emits the [When a Module Has No Compatible Version](module-system.md#when-a-module-has-no-compatible-version) "no version compatible with epoch E" error, naming the highest epoch the module *is* recorded for.
...
## Catalog CI and the compatibility matrix
...
### The per epoch verify and tag loop
...
A community module **may have its own dependencies**, but its compatibility with E is **transitive**: if any dependency has no E-compatible version, the module itself is incompatible with E ([When a Module Has No Compatible Version](module-system.md#when-a-module-has-no-compatible-version) applies to it too). The verification engine enforces this automatically — a failing transitive dep surfaces as a clean gate, not a raw compiler error.
...
- **`prerelease.yml`** — when a `next` epoch is cut: `promise install next` → `promise package build-index . <next-epoch> -report`, posting the unsupported-module list (the [When a Module Has No Compatible Version](module-system.md#when-a-module-has-no-compatible-version) nudge).
