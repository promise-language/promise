...
func elvisFunc(t *testing.T, body string) string {
	t.Helper()
	ir := codegentest.GenerateIR(t, "sink(int n) { }\ndemo() {\n"+body+"\n}\n")
	return codegentest.ExtractFunction(ir, "__user.demo")
}
...
func TestT0935VectorInlineElvisPathFlag(t *testing.T) {
	...
	codegentest.AssertContainsMatch(t, fn, elvisOwnsBoth)
	...
	codegentest.AssertContains(t, fn, "call void @Vector.drop")
}
...
func TestT0935GenericInlineElvisTypeSubst(t *testing.T) {
	...
	fn := codegentest.ExtractDefine(ir, `"inline_len[int]"`)
	if fn == "" {
...
func TestT0935StructuralDefaultInlineElvisSelfSubst(t *testing.T) {
	...
	fn := codegentest.ExtractDefine(ir, "Box.pick_len")
	if fn == "" {
...
func TestT0945BorrowedParamInlineElvisNoDrop(t *testing.T) {
	...
	`)
	fn := codegentest.ExtractDefine(ir, `"borrowed_len[int]"`)
	if fn == "" {
