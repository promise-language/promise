...
type Compiler struct {
	...
	demandedMonoInstances map[string]bool   // mono instance keys demanded by user code (T0207: nil = no filtering)
...
func (c *Compiler) compileModules() {
	...
	// Map is in std), those must be passed to module B's compilation. But a module's
	// own instances are NOT propagated back to itself — the seed filter in
	// collectMonoInstancesWithExtra decides what's needed based on user code demand.
	// This prevents std-internal instances (e.g., Vector[string] from string.split())
	// from being monomorphized when user code doesn't need them. (T0207)
	for _, modInfo := range c.info.ModuleInfos {
...
func (c *Compiler) compileModule(modInfo *sema.ModuleInfo, extraInstances []*types.Instance, extraFuncInstances []*sema.FuncInstance, extraMethodInstances []*sema.MethodInstance) {
	...
	// Compute demanded instances: only extras-demanded instances get their method
	// bodies defined. Module-internal instances (e.g., _FnIter[u8] from Iterator
	// combinator expansion) get layouts and declarations but not method bodies,
	// reducing IR size without breaking references. (T0207)
	c.demandedMonoInstances = computeDemandedInstances(extraInstances, modFile, modInfo, c.spiralInstances)
	...
	c.demandedMonoInstances = nil // clear module-specific filter (T0207)
}
