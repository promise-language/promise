...
func TestT1298VarDeclValueBoxesBeforeOptionalWrap(t *testing.T) {
	...
	codegentest.AssertContains(t, fn, "insertvalue { i1, { i8*, i8* } }")
...
func TestT1298CtorFieldHeapBoxesWithStructuralDrop(t *testing.T) {
	...
	codegentest.AssertContains(t, ir, "@promise_vtable_Heavy_as_Sink")
	...
	cf := codegentest.ExtractFunction(ir, "__user.cf")
	if cf == "" {
		t.Fatalf("could not extract @__user.cf from IR:\n%s", ir)
	}
	codegentest.AssertContains(t, cf, "@promise_vtable_Heavy_as_Sink")
	codegentest.AssertContains(t, cf, "insertvalue { i1, { i8*, i8* } }")
}
...
func TestT1298MapIndexAssignBoxesBeforeSetterCall(t *testing.T) {
	...
	`)
	fn := codegentest.ExtractFunction(ir, "__user.mv")
	if fn == "" {
