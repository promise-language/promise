...
func TestT1558ValueTypeElementBoxesIntoView(t *testing.T) {
	ir := generateIR(t, `
		...
		sink(Tagged[] v) {}
	`)
	fn := extractFunction(ir, "__user.al")
	if fn == "" {
	...
	assertContains(t, fn, "@promise_vtable_Seat_as_Tagged")
	// The boxed view stored into each element slot is the {i8*, i8*} struct.
	assertContains(t, fn, "store { i8*, i8* }")
}
...
func TestT1558HeapElementUsesStructuralDrop(t *testing.T) {
	ir := generateIR(t, `
		...
		sink(Tagged[] v) {}
	`)
	assertContains(t, ir, "@promise_vtable_Booth_as_Tagged")
