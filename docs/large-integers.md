...
### Why not an integer width parameter
...
Add six new universe-scope native types following the existing primitive pattern from [Primitive Types Are Regular Types](language-design.md#primitive-types-are-regular-types) of [language-design.md](language-design.md):
...
Extend [Hashable Ordered Equal and Cloneable](#hashable-ordered-equal-and-cloneable) ("Typed Numeric Literal Suffixes"): the integer suffix list grows to `i8`, `u8`, `i16`, `u16`, `i32`, `u32`, `i64`, `u64`, **`i128`, `u128`, `i256`, `u256`, `i512`, `u512`**.
...
### Soft arithmetic library dependencies
...
- **`to_string` / `parse`**: written in Promise, calling into a small set of helpers (one per width, see [Format and parse](#format-and-parse)) rather than per-type IR.
...
### Hashable Ordered Equal and Cloneable
