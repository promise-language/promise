---
name: next
description: Select the next open item (task or bug) to work on. If tags are given, focus on the items with this tags. Do your best to select the optimal next item to work on from all open items. To minimize merge conflicts, avoid items sharing similar thats with other "in-progress" items. Items tagged with platform name (like windows, poxis, linux) can only be worked on if the host matches the platform. After selecting the item to work on chain to /do.
---
