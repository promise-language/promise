...
func TestT0766LocalChainedDefaultSynth(t *testing.T) {
	...
	if codegentest.ExtractFunction(ir, "Logger.emit_str") == "" {
		t.Error("expected synthesized Logger.emit_str to be emitted")
	}
	if codegentest.ExtractFunction(ir, "Logger.emit_line") == "" {
	...
	codegentest.AssertContains(t, codegentest.ExtractFunction(ir, "Logger.emit_line"), "@Logger.emit_str")
}
...
func TestT0766SiblingDefaultDeclaredAfter(t *testing.T) {
	...
	el := codegentest.ExtractFunction(ir, "Logger.emit_line")
	if el == "" {
		t.Fatal("expected synthesized Logger.emit_line to be emitted")
	}
	codegentest.AssertContains(t, el, "@Logger.emit_str")
	codegentest.AssertContains(t, ir, "@promise_vtable_Logger_as_Sink")
}
...
func TestT0766StdWriterDefaultChain(t *testing.T) {
	...
	if codegentest.ExtractFunction(ir, "Tracked.write_string") == "" {
		t.Error("expected synthesized Tracked.write_string to be emitted")
	}
	wl := codegentest.ExtractFunction(ir, "Tracked.write_line")
	if wl == "" {
		t.Fatal("expected synthesized Tracked.write_line to be emitted")
	}
	codegentest.AssertContains(t, wl, "@Tracked.write_string")
	// View-vtable synthesis must complete (the panic fired before this global
	// was emitted).
	codegentest.AssertContains(t, ir, "@promise_vtable_Tracked_as_Writer")
}
