...
| B0042 | `go extern_func()` panics in codegen: `store operands are not compatible: src=void; dst=i8**` at `genGoCallExpr` (expr.go). Extern functions use sret ABI (void return + first param is sret pointer), but `genGoCallExpr` assumes the call returns a value to store into the task result slot. Affects any `go` expression calling an extern-attributed function that returns a non-void Promise type. Workaround: wrap the extern in a regular Promise function. | 5c | Medium |
