...
func argTypeNameSet(t *testing.T, src, monoNameWanted string) []string {
	t.Helper()
	_, info := codegentest.ParseWithStd(t, src)
	var inst *types.Instance
	for _, in := range codegen.CollectMonoInstances(info) {
		if codegen.MonoName(in) == monoNameWanted {
			inst = in
			break
		}
	}
	if inst == nil {
		var have []string
		for _, in := range codegen.CollectMonoInstances(info) {
			have = append(have, codegen.MonoName(in))
		}
		t.Fatalf("instance %q not found; have %v", monoNameWanted, have)
	}
	var names []string
	for _, tn := range codegen.CollectArgTypeNames(inst) {
...
func TestT0469_NoFnIterDropStubForNonIterGenerics(t *testing.T) {
	...
	codegentest.AssertNotContainsMatch(t, ir, `define void @"Vector\[[^"]*\]\.drop"`)
}
...
func TestT0469_FnIterDropStubPreserved(t *testing.T) {
	...
	stub := codegentest.ExtractDefine(ir, `"_FnIter[int].drop"`)
	if stub == "" {
		t.Fatalf("_FnIter[int].drop stub not found in IR:\n%s", ir)
	}
	codegentest.AssertContains(t, stub, "call void @__promise_iter_cleanup")
}
...
func TestT0469_NativeGenericDropsAreTypeSpecific(t *testing.T) {
	...
	chanIR := codegentest.GenerateIR(t, `
		main() {
			c := channel[int](2);
			c.send(1);
			c.close();
	...
	for _, name := range []string{`"Channel[int].drop"`, `"Task[int].drop"`} {
		drop := codegentest.ExtractDefine(chanIR, name)
		if drop == "" {
	...
	refIR := codegentest.GenerateIR(t, `
	...
	`)
	refDrop := codegentest.ExtractDefine(refIR, `"Ref[int].drop"`)
	if refDrop == "" {
	...
	codegentest.AssertContains(t, refDrop, "atomicrmw")
}
