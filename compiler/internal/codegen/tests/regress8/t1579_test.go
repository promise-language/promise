...
func TestT1579RepeatOptionalElement(t *testing.T) {
	ir := codegentest.GenerateIR(t, `main() { u32?[4] w = [0u32; 4]; }`)
	codegentest.AssertContains(t, ir, "alloca [4 x { i1, i32 }]")
	// The stored value is the wrapped optional struct, not a bare i32.
	codegentest.AssertContains(t, ir, "store { i1, i32 }")
}

// Nested repeat literals produce nested fixed arrays.
func TestT1579NestedRepeat(t *testing.T) {
	ir := codegentest.GenerateIR(t, `main() { u8[4][3] grid = [[1u8; 4]; 3]; }`)
	codegentest.AssertContains(t, ir, "alloca [3 x [4 x i8]]")
	codegentest.AssertContains(t, ir, "alloca [4 x i8]")
}
