...
func TestT1182_ArrayInlineUnwrapDoesNotDoubleDrop(t *testing.T) {
	ir := codegentest.GenerateIR(t, t1182Decls+`
		afu() int {
		...
		main() { _ := afu(); }
	`)
	fn := codegentest.ExtractFunction(ir, "__user.afu")
	if fn == "" {
...
func TestT1182_VectorInlineUnwrapDoesNotDoubleDrop(t *testing.T) {
	ir := codegentest.GenerateIR(t, t1182Decls+`
		...
		main() { _ := vfu(); }
	`)
	fn := codegentest.ExtractFunction(ir, "__user.vfu")
	if fn == "" {
...
func TestT1182_ArrayBindingFormDups(t *testing.T) {
	ir := codegentest.GenerateIR(t, t1182Decls+`
		...
		main() { _ := abind(); }
	`)
	fn := codegentest.ExtractFunction(ir, "__user.abind")
	if fn == "" {
...
func TestT1182_VectorBindingFormDups(t *testing.T) {
	ir := codegentest.GenerateIR(t, t1182Decls+`
		...
		main() { _ := vbind(); }
	`)
	fn := codegentest.ExtractFunction(ir, "__user.vbind")
	if fn == "" {
