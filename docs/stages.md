...
| B0041 | Goroutines spawned in batch test functions never get scheduled. `go` expressions inside `` `test ``-annotated functions deadlock — the spawned goroutine sits in the run queue but the scheduler never resumes it. Reproducer: `test_go() \`test { t := go get_value(); int x = <-t; }` hangs indefinitely. Works correctly in snapshot tests (`main() \`test(expected: ...)`) because they use a real `main()` entry point that bootstraps the scheduler normally. Root cause likely: the batch test harness invokes test functions without the full scheduler bootstrap (sched_init → sched_loop) that `main()` performs, so spawned Gs are enqueued but no M picks them up. Workaround: use PAL-level pthreads for concurrent operations in batch tests, or use snapshot tests for concurrency tests. | 5c | High |
