...
| B0027 | `os.execute` reads stdout/stderr sequentially — deadlock if child writes >64KB to stderr while stdout pipe is not drained. Fix: read both pipes concurrently (second thread or `poll`/`select` multiplexing in PAL `EmitExecute`). | E | Medium |
