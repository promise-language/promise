...
func TestT1377TransitiveModuleIteratorVtableHasNoNullSlots(t *testing.T) {
	...
	assertNotContains(t, vtable, "@MyIter__int.count")
}
...
func TestT1377MonoTransitiveDefaultDefined(t *testing.T) {
	...
	vtable := extractGlobal(ir, "promise_vtable_MyBox[int]")
	if vtable == "" {
	...
	assertContains(t, vtable, `@"MyBox[int].count"`)
	...
	assertContains(t, body, `@"MyBox[int].next"`)
}
