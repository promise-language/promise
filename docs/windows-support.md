...
- **File I/O:** open, read, write, close, seek, stat, mkdir, rmdir, dir listing — opened with `FILE_SHARE_DELETE` so an open file can be removed (T1742), and renamed over via the POSIX-semantics rename (T1930)
...
| Rename | `pal_file_rename` | MoveFileExA (REPLACE_EXISTING\|WRITE_THROUGH), falling back to SetFileInformationByHandle(FileRenameInfoEx, POSIX_SEMANTICS) for an open destination |
