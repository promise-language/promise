...
> **Status: Implemented.** All three phases are done (Phase 1: local modules + visibility; Phase 2: transitive deps + diamond dedup; Phase 3: remote git fetch + `promise pin`). This document is the authoritative design reference. See `docs/stages.md` Stage 9 for implementation status details.
...
- **All caches are global** (`~/.promise/cache/`). All cache keys are content-addressed using FNV-128a — module build keys (impl hash + compiler hash + target + module paths), test binary keys (source content + compiler hash + std hash + target + local module deps). FNV-128a is used throughout for CAS hashing; SHA-256 is reserved for externally-verified checksums (e.g., downloaded release artifacts). Same inputs always produce the same cache key regardless of which project triggered the build.
