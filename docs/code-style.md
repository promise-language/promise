...
- **No marker, and no guard.** Unlike the three sections around it this rule has no pre-commit check, deliberately: the defect shape is `got != want` over two `string` values, which nothing can tell apart by grep from every correct string comparison in the tree. Enforcement here is execution — the 8.3 pair gets built and asserted on — not lint.
...
- **`// path-ok: <reason>` annotates a legitimate lookup.** A pre-commit guard (`CheckHostToolLookups` in `tools/build/common/hosttoolcheck.go`, also run over the real tree by the tools test suite) rejects any tracked `.go` line that calls `Which`/`exec.LookPath`, or names a toolchain binary bare to `exec.Command`, unless that line carries the marker with a non-empty reason. `git`, `sh`, `go` and `gofmt` need none — they are the environment the build runs inside, not tools it builds with. Per-line like its two siblings, and for the same reason.

- **The three reasons that hold.** A tool whose subject *is* host state (`bin/prereqs`, `promise doctor`); a runtime that executes a built artifact without contributing to it (`wasmtime`, `node` — see [gate-system.md](gate-system.md)); and an explicitly requested non-default target that links against the host's own runtime (`findCRT` for `--target *-linux-gnu`). Anything else is a resolver that should be reading the pinned set.
