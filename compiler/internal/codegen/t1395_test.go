...
func TestT1395_ExtraIntDefaultFilledInGenericDispatch(t *testing.T) {
	...
	body := extractDefine(ir, "scan[Widget]")
	if body == "" {
...
func TestT1395_OptionalNoDefaultFilledAsNone(t *testing.T) {
	...
	ir := generateIR(t, `
		...
		main() { run(); }
	`)
	body := extractDefine(ir, "scan[Tagged]")
	if body == "" {
...
func TestT1395_MatchingAritySignatureUnchanged(t *testing.T) {
	ir := generateIR(t, `
		...
		main() { run(); }
	`)
	body := extractDefine(ir, "scan[Plain]")
	if body == "" {
...
func TestT1395_VirtualDispatchFillsDefault(t *testing.T) {
	ir := generateIR(t, `
		...
		main() { run(); }
	`)
	body := extractDefine(ir, "render[Parent]")
	if body == "" {
...
func TestT1395_EnumDispatchFillsDefault(t *testing.T) {
	ir := generateIR(t, `
		...
		main() { run(); }
	`)
	body := extractDefine(ir, "render[Level]")
	if body == "" {
...
func TestT1395_IterNextDirectDispatchFillsDefault(t *testing.T) {
	ir := generateIR(t, `
		...
		main() { run(); }
	`)
	body := extractDefine(ir, "__user.run")
	if body == "" {
...
func TestT1395_ValueTypeDispatchFillsDefault(t *testing.T) {
	ir := generateIR(t, `
		...
		main() { run(); }
	`)
	body := extractDefine(ir, "render[Coord]")
	if body == "" {
...
func TestT1395_MultipleTrailingDefaultsFilledInOrder(t *testing.T) {
	ir := generateIR(t, `
		...
		main() { run(); }
	`)
	body := extractDefine(ir, "render[Combo]")
	if body == "" {
