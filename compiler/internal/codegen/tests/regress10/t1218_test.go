...
func TestT1218_WasmNonCoroutineMutexLockPumpsCoopStep(t *testing.T) {
	ir := codegentest.GenerateIRForTarget(t, t1218MutexSrc, "wasm32-wasi")

	grab := codegentest.FindDefinedFunc(ir, "@__user.grab(")
	if grab == "" {
...
func TestT1218_NativeMutexLockUnchanged(t *testing.T) {
	ir := codegentest.GenerateIRForTarget(t, t1218MutexSrc, "x86_64-unknown-linux-gnu")
