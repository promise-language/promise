...
| B0026 | PAL `EmitFileStatSize` depends on `lookupFunc` for `pal_file_open`/`pal_file_close` — will panic if emission order in `declareIntrinsics()` changes. Consider explicit func parameters or nil-check with clear error. Same pattern in Windows `EmitFileStatSize`. | D | Low |
