...
7. **If nothing fits, coin it — and record it here in the same change (§5).** The vocabulary is meant to grow; what it must never do is grow *silently*. A tag that exists on items but not in this file is invisible to everyone who reads this file first, which is how a namespace rots into synonyms.
...
When filing, ask: **one subsystem-or-area tag? (bugs) one kind tag? platform only if platform-specific? all canonical spellings? ≤ 5 total?** If a tag you need does not exist, add it here in the same change (§5). If yes, file it.
