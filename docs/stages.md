...
| B0043 | `for k, v in map[K, Enum]` crashes **cross-module** — works in same-file tests (B0030 fix), but panics with `undeclared map keys/values method` when the enum is defined in a catalog module (e.g., `JsonValue` in `json` module) and used by an external file. Cross-module monomorphization doesn't propagate `Map[K, Enum].keys()`/`values()` method instances. Blocks T0001. Workaround: parallel arrays. | Codegen | High |
