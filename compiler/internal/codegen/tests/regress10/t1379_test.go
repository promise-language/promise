...
func TestT1379_FailableReceiveWasmTimeoutMerge(t *testing.T) {
	...
	codegentest.AssertContains(t, ir, "task.recv_merge")
	codegentest.AssertContains(t, ir, `@"FailableTask[int].free_after_done"`)
	codegentest.AssertContains(t, ir, "error.panic")
}
...
func TestT1379_PlainTaskUnaffected(t *testing.T) {
	...
	`)
	codegentest.AssertContains(t, ir, `@"Task[int].drop"`)
	if strings.Contains(ir, `@"FailableTask[int].drop"`) {
...
func TestT1381_DrainTasksLowering(t *testing.T) {
	...
	`)
	codegentest.AssertContains(t, ir, "drain.head")
	codegentest.AssertContains(t, ir, "drain.build.err")
	// Successes are pushed onto the fresh accumulator vector.
	codegentest.AssertContains(t, ir, "@promise_vector_push(")
