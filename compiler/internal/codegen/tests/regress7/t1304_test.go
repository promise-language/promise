...
func TestT1304FreshReturnKeepsOwnDrop(t *testing.T) {
	...
	`)
	fn := codegentest.ExtractFunction(ir, "__user.binding_fresh")
	if fn == "" {
	...
	codegentest.AssertNotContains(t, fn, "struct.arg.alias.clear")
}
...
func TestT1304FailableBindingPeelsWrapper(t *testing.T) {
	...
	`)
	fn := codegentest.ExtractFunction(ir, "__user.binding_fail")
	if fn == "" {
...
func TestT1304MultiArgSkipsNonIdent(t *testing.T) {
	...
	`)
	fn := codegentest.ExtractFunction(ir, "__user.binding_multi")
	if fn == "" {
...
func TestT1304ErrorPropagatePeelsWrapper(t *testing.T) {
	...
	`)
	fn := codegentest.ExtractFunction(ir, "__user.binding_propagate")
	if fn == "" {
...
func TestT1304NonCallRHSNoGuard(t *testing.T) {
	...
	`)
	fn := codegentest.ExtractFunction(ir, "__user.binding_ident")
	if fn == "" {
