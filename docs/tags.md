...
3. **Mandatory facets:**
   - **Every item** carries **at least one Component/subsystem _or_ Area tag** ([Component and subsystem](#component-and-subsystem) / [Area and initiative](#area-and-initiative)) — what part of the system it touches.
   - **Every bug** additionally carries **exactly one Quality/kind tag** ([Quality and kind](#quality-and-kind)) — what _kind_ of defect it is (`memory-leak`, `double-free`, `crash`, `flaky`, `correctness`, `performance`, …).
   - **Platform** ([Platform](#platform)) tags are added **only** when the item is genuinely platform-specific.
   - A **Document tag** ([Document](#document)) **does not satisfy** the subsystem/area requirement — it says *which end state the item moves toward*, not *what part of the system it touches*. An item carrying one still needs its own subsystem-or-area tag (and, if a bug, its kind tag): doc tag + subsystem + kind = 3 of the 5 allowed. The exception is a doc tag that also appears in [Area and initiative](#area-and-initiative)'s area list — the two names coincide, so the one tag fills both roles and is counted once.
...
6. **Prefer the canonical spelling ([Canonical spellings](#canonical-spellings)).** If the concept already has a canonical tag, use it; don’t add a synonym.
7. **If nothing fits, coin it — and record it here in the same change ([Extending the namespace](#extending-the-namespace)).** The vocabulary is meant to grow; what it must never do is grow *silently*. A tag that exists on items but not in this file is invisible to everyone who reads this file first, which is how a namespace rots into synonyms. Document tags ([Document](#document)) are the single exception: they are defined by the contents of `docs/` root rather than by a list here, so they are already visible and must not be copied into [The namespace by facet](#the-namespace-by-facet).

---

## The namespace by facet

Pick tags from these facets. Lists are the established vocabulary — extend only when nothing fits, and when you do, follow [Extending the namespace](#extending-the-namespace).
...
- `formatter` ([Component and subsystem](#component-and-subsystem) — the formatter subsystem: lexer, reformatter, its bugs) vs `formatting` ([Document](#document) — the [formatting.md](formatting.md) end state). A formatter bug is `formatter`; work that closes a gap in the formatting spec is both.
- `schema` ([Document](#document) — the [schema.md](schema.md) end state) vs `types` ([Component and subsystem](#component-and-subsystem) — the compiler's type representation). Schema descriptors are built *on* `types`; they are not the same area.
...
The lists in [The namespace by facet](#the-namespace-by-facet) are a starting vocabulary, not a closed set. New subsystems, platforms, and defect kinds appear, and when one does it deserves a tag. Coining one is fine — **leaving it undocumented is not.**

**Add a tag when all of these hold:**

1. **Nothing in [The namespace by facet](#the-namespace-by-facet) fits.** Not "nothing fits perfectly" — nothing fits. A slightly-too-broad existing tag beats a new near-synonym.
2. **It is not a synonym.** Check [Canonical spellings](#canonical-spellings) first. If the concept already has a canonical spelling, use that one; if your word is the better name, change [Canonical spellings](#canonical-spellings)'s table rather than adding a second spelling.
...
1. Tag the item.
2. In the *same* change, append the tag to the right facet list in [The namespace by facet](#the-namespace-by-facet) — component/subsystem, platform, area, or quality/kind. Keep the existing grouping; these lists are ordered by affinity, not alphabetically.
   Document tags ([Document](#document)) are the one facet you do **not** add here: the vocabulary *is* `docs/` root, so a new one appears by adding or ratifying a doc there — never by editing this file. If you find yourself pasting a doc tag into [Document](#document), that is the duality this facet exists to avoid.
3. If it supersedes or competes with an existing tag, add a row to [Canonical spellings](#canonical-spellings) so the losing spelling is explicitly retired rather than left to compete.
...
When filing, ask: **one subsystem-or-area tag? (bugs) one kind tag? platform only if platform-specific? a document tag if the item closes a gap in a `docs/` root spec? all canonical spellings? ≤ 5 total?** If a tag you need does not exist, add it here in the same change ([Extending the namespace](#extending-the-namespace)). If yes, file it.
