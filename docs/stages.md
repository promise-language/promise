...
| ~~B0041~~ | ~~Goroutines spawned in batch test functions never get scheduled.~~ **Fixed.** `GenerateTestMain` now calls `promise_sched_init(num_cpus)` before running tests and `promise_sched_shutdown()` after, so worker Ms exist to pick up goroutines spawned by `go` in batch test functions. Test functions use thread-blocking mode (cond_wait/spin) for channel ops, which works correctly with the scheduler running in background. WASM batch tests with goroutines remain unsupported (cooperative scheduler cannot run goroutines from non-coroutine context). | 5c | ~~High~~ Resolved |
| ~~B0044~~ | ~~`go extern_func()` panics in codegen: `store operands are not compatible: src=void; dst=i8**` at `genGoCallExpr` (expr.go). Extern functions use sret ABI (void return + first param is sret pointer), but `genGoCallExpr` assumes the call returns a value to store into the task result slot~~ — **Fixed.** `genGoCallExpr` now detects extern targets via `c.externs` and generates an auto-wrapper function (`.go_extern_wrap.<name>`) using `saveState`/`restoreState` + `genExternCall` to handle sret/ABI coercion. The coroutine calls the wrapper (which returns the Promise-internal type) instead of the extern directly. | 5c | ~~Medium~~ Resolved |
| B0043 | Blocking select (no default) silently skips in batch tests. When `inCoroutine=false` (batch test functions), the yield-and-retry loop for blocking select is not emitted — control falls through to the merge block immediately without selecting any case. Only affects batch test functions; snapshot tests and normal `main()` are coroutines and work correctly. Workaround: use `select` with a `default` branch, or use snapshot tests for blocking select scenarios. | 5c | Low |
...
| B0019 | Extern ABI for generic types | 8f | Low |
| B0020 | Non-instance field placements: mixed `value`+instance, `variant`/`type` fields, `global`/`mono` data placement | 8c | Low |
...
| B0021 | `yield*` delegate (forward all values from sub-iterator) | Generators | Medium |
| B0022 | Failable generators (`stream[T]!` with error propagation through yield) | Generators | Low |
| B0023 | Stored generator values (first-class generator variables outside for-in) | Generators | Low |
| B0024 | Generator closures (capturing lambdas as generators) | Generators | Low |
...
| # | Item | Origin |
|---|------|--------|
| B0032 | Explicit lifetime annotations | 6b |
| B0033 | Stored references in structs | 6b |
| B0034 | Full NLL last-use analysis | 6b |
| B0035 | Drop ordering | 6b |
| B0036 | Disjoint field borrows | 6b |

### Meta Annotations

| # | Item | Origin |
|---|------|--------|
| B0037 | `inline`, `packed`, `align`, `extern`, `serializable`, `public`, `unsafe` processing | 7 |
...
|---|------|-----------|----------|
| T0001 | `JsonValue` use `map[string, JsonValue]` for Object variant instead of parallel arrays | ~~B0030~~ ~~B0043~~ (resolved) | High |
