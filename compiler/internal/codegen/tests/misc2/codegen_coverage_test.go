...
func TestCoverageFunctionEntry(t *testing.T) {
	ir, regions := codegentest.GenerateIRWithCoverage(t, `
		foo() int { return 42; }
		bar() int { return 7; }
		main() {}
	`)
	// Should have coverage globals for foo and bar (not main)
	codegentest.AssertContains(t, ir, "@__promise_cov_0")
	codegentest.AssertContains(t, ir, "@__promise_cov_1")
...
func TestCoverageSkipsTestFunctions(t *testing.T) {
	_, regions := codegentest.GenerateIRWithCoverage(t, `
		foo() int { return 42; }
		test_foo() `+"`test"+` {
			int x = foo();
		}
...
func TestCoverageIfBranches(t *testing.T) {
	...
	codegentest.AssertContains(t, ir, "@__promise_cov_0")
	codegentest.AssertContains(t, ir, "@__promise_cov_1") // if.then
	codegentest.AssertContains(t, ir, "@__promise_cov_2") // if.else
...
func TestCoverageTestMainEmitsMarkers(t *testing.T) {
	...
	codegentest.AssertContains(t, ir, "===PROMISE_COV===")
	codegentest.AssertContains(t, ir, "===END_COV===")
...
func TestCoverageGenericMethodInstanceShared(t *testing.T) {
	...
	codegentest.AssertContains(t, mainIR, g+" = global i64 0")
	codegentest.AssertNotContains(t, mainIR, g+" = private global")
	...
	codegentest.AssertContains(t, instIR, g+" = external global")
	...
	codegentest.AssertNotContains(t, instIR, g+" = global i64 0")
	codegentest.AssertNotContains(t, instIR, g+" = private global")
}
...
func TestCompileResultCoverageEnabled(t *testing.T) {
	...
	if got := codegen.CompileWithOptions(file, info, "", &codegen.CompileOptions{CoverageEnabled: true}).CoverageEnabled(); !got {
		t.Errorf("CoverageEnabled() = false, want true when CompileOptions.CoverageEnabled is set")
	}

	file2, info2 := codegentest.ParseWithStd(t, src)
	if got := codegen.CompileWithOptions(file2, info2, "", &codegen.CompileOptions{CoverageEnabled: false}).CoverageEnabled(); got {
		t.Errorf("CoverageEnabled() = true, want false when CompileOptions.CoverageEnabled is explicitly false")
	}

	file3, info3 := codegentest.ParseWithStd(t, src)
	if got := codegen.Compile(file3, info3, "").CoverageEnabled(); got {
