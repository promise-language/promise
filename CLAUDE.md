...
- **Prefer batch tests over snapshot tests.** Promise has two test styles:
  ...
  **Memory leak enforcement** (T0067): Tests that leak memory (alloc count delta > 0) cause test failure. Tests with known leaks can be tagged `` `test(allow_leaks: true) `` to suppress the failure while still reporting the leak. The test summary shows `N ignored` for allow_leaks tests that leaked, and prints a warning when a tagged test does NOT leak (stale tag). The goal is to progressively remove all `allow_leaks` tags as leak sources are fixed (T0066).
