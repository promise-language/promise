...
func TestT1308MethodBindingClearsArgAlias(t *testing.T) {
	...
	`)
	fn := codegentest.ExtractFunction(ir, "__user.method_binding_passthrough")
	if fn == "" {
...
func TestT1308MethodFreshReturnKeepsOwnDrop(t *testing.T) {
	...
	`)
	fn := codegentest.ExtractFunction(ir, "__user.method_binding_fresh")
	if fn == "" {
...
func TestT1308MethodMultiArgEmitsGuardPerArg(t *testing.T) {
	...
	`)
	fn := codegentest.ExtractFunction(ir, "__user.method_multi_arg")
	if fn == "" {
