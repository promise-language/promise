...
func TestT0655MutexTempReceiverPromotedToScopeBinding(t *testing.T) {
	ir := codegentest.GenerateIR(t, t0655Src)
	...
	for _, fnName := range []string{
		...
		fn := codegentest.ExtractFunction(ir, fnName)
		if fn == "" {
...
func TestT0655BoundReceiverUnchanged(t *testing.T) {
	ir := codegentest.GenerateIR(t, t0655Src)
	fn := codegentest.ExtractFunction(ir, "__user.use_bound")
	if fn == "" {
...
func TestT0655GenericMutexTempPromoted(t *testing.T) {
	...
	fn := codegentest.ExtractFunction(ir, `"glk[int]"`)
	if fn == "" {
...
func TestT0655ConsumeOnlyUnchanged(t *testing.T) {
	ir := codegentest.GenerateIR(t, t0655Src)
	fn := codegentest.ExtractFunction(ir, "__user.use_consume")
	if fn == "" {
