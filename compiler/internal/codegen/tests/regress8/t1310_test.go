...
func TestT1310MethodDiscardClearsArgAlias(t *testing.T) {
	...
	`)
	fn := codegentest.ExtractFunction(ir, "__user.method_discard")
	if fn == "" {
...
func TestT1310MethodInlineUseClearsArgAlias(t *testing.T) {
	...
	`)
	fn := codegentest.ExtractFunction(ir, "__user.method_inline")
	if fn == "" {
...
func TestT1310MethodDiscardMultiArgEmitsGuardPerArg(t *testing.T) {
	...
	`)
	fn := codegentest.ExtractFunction(ir, "__user.method_two_args")
	if fn == "" {
...
func TestT1310MethodDiscardParenArgClearsAlias(t *testing.T) {
	...
	`)
	fn := codegentest.ExtractFunction(ir, "__user.method_paren")
	if fn == "" {
...
func TestT1310GenericMethodDiscardClearsArgAlias(t *testing.T) {
	...
	`)
	fn := codegentest.ExtractFunction(ir, "__user.method_generic")
	if fn == "" {
...
func TestT1310MethodDiscardFreshReturnNoGuard(t *testing.T) {
	...
	`)
	fn := codegentest.ExtractFunction(ir, "__user.method_fresh")
	if fn == "" {
	...
	codegentest.AssertNotContains(t, fn, "struct.tmp.alias.clear")
}
