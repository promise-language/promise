...
func TestT1527ValueNewtypeSharesParentValueStruct(t *testing.T) {
	ir := codegentest.GenerateIR(t, t1527Src)
	// The child declares no value struct of its own — it reuses the parent's,
	// which is what makes every upcast a no-op.
	codegentest.AssertNotContains(t, ir, "%promise_EntityId_v = type")
	codegentest.AssertContains(t, ir, "%promise_Hash128_v = type { i8*, i128 }")
	// It still gets its own RTTI identity (own _t/_m/_i + typeinfo + rtti global).
	codegentest.AssertContains(t, ir, "%promise_EntityId_i = type")
	codegentest.AssertContains(t, ir, "@promise_typeinfo_EntityId")
	codegentest.AssertContains(t, ir, "@promise_rtti_EntityId")
}
...
func TestT1527ValueNewtypeConstructorIsInlineNoAlloc(t *testing.T) {
	...
	codegentest.AssertContains(t, ir, "insertvalue %promise_Hash128_v")
	codegentest.AssertContains(t, ir, "alloca %promise_Hash128_v")
	codegentest.AssertNotContains(t, ir, "%promise_EntityId_i*")
	codegentest.AssertNotContains(t, ir, "%promise_Hash128_i*")
}
...
func TestT1527ValueNewtypeDispatchesStatically(t *testing.T) {
	...
	codegentest.AssertContains(t, ir, "call i128 @Hash128.half(")
	codegentest.AssertContains(t, ir, "call i64 @EntityId.tag(")
	codegentest.AssertNotContains(t, ir, "vtable.slot")
}
...
func TestT1527ValueNewtypeUpcastNeedsNoCoercion(t *testing.T) {
	...
	codegentest.AssertContains(t, ir, "store %promise_Hash128_v")
	codegentest.AssertContains(t, ir, "call i128 @__user.widen(%promise_Hash128_v")
}
...
func TestT1527ValueNewtypeChainSharesRootValueStruct(t *testing.T) {
	...
	`)
	codegentest.AssertContains(t, ir, "%promise_A_v = type { i8*, i64 }")
	codegentest.AssertNotContains(t, ir, "%promise_B_v = type")
	codegentest.AssertNotContains(t, ir, "%promise_C_v = type")
	codegentest.AssertContains(t, ir, "@promise_rtti_C")
}
...
func TestT1527ValueNewtypeHeaderDefinesSharedStructOnce(t *testing.T) {
	result := codegentest.CompileResult(t, t1527Src)
...
func TestT1527ValueNewtypeAsEnumVariantPayload(t *testing.T) {
	...
	codegentest.AssertContains(t, ir, "%promise_Ticket_v = type { i8*, i64 }")
	codegentest.AssertNotContains(t, ir, "%promise_Seat_v = type")
	codegentest.AssertContains(t, ir, "{ %promise_Ticket_v }")
}
...
func TestT1527ValueNewtypeAsValueFieldEmbedsParentStruct(t *testing.T) {
	...
	`)
	codegentest.AssertContains(t, ir, "%promise_V_v = type { i8*, i64 }")
	codegentest.AssertContains(t, ir, "%promise_Wrap_v = type { i8*, %promise_V_v }")
	codegentest.AssertNotContains(t, ir, "%promise_W_v = type")
}
...
func TestT1527ValueNewtypeInheritedMutatorUsesCallerStorage(t *testing.T) {
	...
	codegentest.AssertContains(t, ir, "%w = alloca %promise_V_v")
	codegentest.AssertNotContains(t, ir, "alloca %promise_W_v")
	codegentest.AssertContains(t, ir, "bitcast %promise_V_v* %w to i8*")
	codegentest.AssertContains(t, ir, "call void @V.set_n(")
}
...
func TestT1527ValueNewtypeOfModuleValueParent(t *testing.T) {
	...
	`)
	codegentest.AssertContains(t, ir, "%promise_Nanos_v = type { i8*, i64 }")
	codegentest.AssertNotContains(t, ir, "%promise_Timeout_v = type")
	codegentest.AssertContains(t, ir, "@promise_rtti_Timeout")
	codegentest.AssertContains(t, ir, "call i64 @__mod_mylib_Nanos.doubled(")
}
