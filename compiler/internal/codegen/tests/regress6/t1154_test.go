...
func TestT1154_GoCallEnumPayloadBorrowDropsInGoroutine(t *testing.T) {
	ir := codegentest.GenerateIR(t, `
		...
		main() { probe(); }
	`)
	coro := codegentest.ExtractDefine(ir, ".goroutine.0")
	if coro == "" {
	...
	codegentest.AssertContains(t, coro, "@__user.msg_len")
	codegentest.AssertContains(t, coro, "call void @Msg.drop")
}
...
func TestT1154_GoCallEnumPayloadMoveNoGoroutineDrop(t *testing.T) {
	...
	codegentest.AssertNotContains(t, coro, "call void @Msg.drop")
}
