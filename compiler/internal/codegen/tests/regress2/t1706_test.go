...
func TestFieldDropPolymorphicUsesRtti(t *testing.T) {
	...
	`)
	boxDrop := codegentest.ExtractDefine(ir, "Box.drop$synth")
	if boxDrop == "" {
		boxDrop = codegentest.ExtractDefine(ir, "Box.drop")
	}
	if boxDrop == "" {
...
func TestArcInnerDropPolymorphicUsesRtti(t *testing.T) {
	...
	`)
	arcDrop := codegentest.ExtractDefine(ir, "Ref[Base].drop")
	if arcDrop == "" {
...
func TestMutexInnerDropPolymorphicUsesRtti(t *testing.T) {
	...
	`)
	mutexDrop := codegentest.ExtractDefine(ir, "Mutex[Base].drop")
	if mutexDrop == "" {
...
func TestArcInnerDropStructuralUsesRtti(t *testing.T) {
	...
	`)
	arcDrop := codegentest.ExtractDefine(ir, "Ref[Showable].drop")
	if arcDrop == "" {
...
func TestMutexInnerDropStructuralUsesRtti(t *testing.T) {
	...
	`)
	mutexDrop := codegentest.ExtractDefine(ir, "Mutex[Showable].drop")
	if mutexDrop == "" {
...
func TestFieldDropConcreteBaseWithChildrenUsesRtti(t *testing.T) {
	...
	`)
	kennelDrop := codegentest.ExtractDefine(ir, "Kennel.drop$synth")
	if kennelDrop == "" {
		kennelDrop = codegentest.ExtractDefine(ir, "Kennel.drop")
	}
	if kennelDrop == "" {
...
func TestFieldDropBaseNoDrop_ChildAddsFields_UsesRtti(t *testing.T) {
	...
	`)
	holderDrop := codegentest.ExtractDefine(ir, "Holder.drop$synth")
	if holderDrop == "" {
		holderDrop = codegentest.ExtractDefine(ir, "Holder.drop")
	}
	if holderDrop == "" {
