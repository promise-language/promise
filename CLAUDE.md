...
- **Shutdown signal/join**: `promise_sched_shutdown` uses `max_p` (not `num_p`) for its signal and join loops, ensuring Ms on disabled Ps (after `set_max_procs` reduced `num_p`) are still signaled and joined. Does NOT free the P array or destroy mutexes — the sysmon thread (not joined, T0022) may still be scanning Ps. Process exit reclaims all resources.
