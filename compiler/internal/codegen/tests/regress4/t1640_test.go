...
func TestT1640GoBlockClosureCaptureTransfersEnv(t *testing.T) {
	...
	codegentest.AssertContains(t, coro, "f.dropflag")
	codegentest.AssertContains(t, coro, "env.free")
	codegentest.AssertContains(t, coro, "call void @pal_free")
	...
	codegentest.AssertContains(t, ir, "store i1 false, i1* %f.dropflag")
}
...
func TestT1640GoBlockCaptureFreeClosureStillTransfers(t *testing.T) {
	...
	codegentest.AssertContains(t, coro, "env.skip")
}
...
func TestT1640ClosureReassignHasNoGenericDropOld(t *testing.T) {
	...
	`)
	body := codegentest.FnBodyT0913(t, ir, "reassign_t1640")
	codegentest.AssertContains(t, body, "reassign.env.free")
	codegentest.AssertNotContains(t, body, "%drop.exec")
}
...
func TestT1640MixedClosureAndStringCapturesUseDistinctPaths(t *testing.T) {
	...
	`)
	coro := extractGoCoroutineBody(t, ir)
	codegentest.AssertContains(t, coro, "env.free")             // closure capture
	codegentest.AssertContains(t, coro, "@promise_string_drop") // string capture
	// Both outer bindings are surrendered by the spawner.
	codegentest.AssertContains(t, ir, "store i1 false, i1* %f.dropflag")
	codegentest.AssertContains(t, ir, "store i1 false, i1* %s.dropflag")
}
...
func TestT1640VectorOfClosuresCaptureUsesDropPath(t *testing.T) {
	...
	`)
	coro := extractGoCoroutineBody(t, ir)
	codegentest.AssertContains(t, coro, "fns.dropflag")
	codegentest.AssertNotContains(t, coro, "fns.env.free")
	codegentest.AssertContains(t, ir, "store i1 false, i1* %fns.dropflag")
}
