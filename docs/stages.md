...
| ~~PAL file I/O functions emitted but not yet wired to Promise-level `io` module~~ — **Done.** `modules/io/io.pr` implements `File` (4 factory constructors, handle methods, one-shot helpers), `Dir` (make/make_all/list/remove/exists), `IoError`, `read_line`, `read_stdin`. 54 tests in `io_test.pr`. | D | ~~Low~~ Resolved |
