...
func TestConfinedRefDropIsNonAtomic(t *testing.T) {
	...
	`)
	dropFn := codegentest.ExtractFunction(ir, `"Ref[Data].drop"`)
	if dropFn == "" {
...
func TestConfinedWeakDropIsNonAtomic(t *testing.T) {
	...
	`)
	weakDrop := codegentest.ExtractFunction(ir, `"Weak[Data].drop"`)
	if weakDrop == "" {
...
func TestConfinedEnumRefIsNonAtomic(t *testing.T) {
	...
	`)
	dropFn := codegentest.ExtractFunction(ir, `"Ref[Color].drop"`)
	if dropFn == "" {
