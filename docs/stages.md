...
| ~~B-003~~ | ~~`\{` escape in string literals~~ — **Done.** The lexer tokenizes `\{` as `STRING_ESCAPE`, and `resolveEscape` converts it to a literal `{`. Works in both static strings and interpolated strings (e.g., `"a\{b{x}c"`). | Codegen | ~~High~~ Resolved |
