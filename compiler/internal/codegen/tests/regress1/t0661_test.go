...
func TestT0661_ContainsBareIntoOptionalVectorWraps(t *testing.T) {
	ir := codegentest.GenerateIR(t, `main() { int?[] v = []; v.push(1); bool b1 = v.contains(1); bool b2 = v.contains(none); }`)
	body := codegentest.ExtractFunc(ir, ".goroutine.main")
	if body == "" {
...
func TestT0661_ContainsExplicitOptionalNoDoubleWrap(t *testing.T) {
	ir := codegentest.GenerateIR(t, `
		...
		main() { int? o = 5; _ := f(o); }
	`)
	body := codegentest.ExtractFunction(ir, "__user.f")
	if body == "" {
...
func TestT0661_ContainsBareIntoGenericOptionalVector(t *testing.T) {
	ir := codegentest.GenerateIR(t, `
		...
		main() { _ := _check[int](7); }
	`)
	body := codegentest.ExtractFunc(ir, `"_check[int]"`)
	if body == "" {
...
func TestT0661_ContainsBareF64IntoOptionalVectorCustomEqFn(t *testing.T) {
	ir := codegentest.GenerateIR(t, `main() { f64?[] v = []; v.push(3.5); bool b = v.contains(3.5); }`)

	if !strings.Contains(ir, "@__promise_opt_eq_double(") {
		t.Fatalf("expected @__promise_opt_eq_double in module IR:\n%s", ir)
	}
	eqBody := codegentest.ExtractFunc(ir, "__promise_opt_eq_double")
	if eqBody == "" {
...
func TestT0661_ContainsOptionalUsesCustomEqFn(t *testing.T) {
	ir := codegentest.GenerateIR(t, `main() { int?[] v = []; v.push(1); bool b = v.contains(1); }`)
	...
	eqBody := codegentest.ExtractFunc(ir, "__promise_opt_eq_i64")
	if eqBody == "" {
