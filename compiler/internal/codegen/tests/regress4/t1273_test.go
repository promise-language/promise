...
func TestWriteBytesBody(t *testing.T) {
	...
	`)
	codegentest.AssertContains(t, ir, "define void @promise_write_bytes(i8* %sret, i8* %fd, i8* %buf)")
	// fd is dynamic (extracted from the int param), then pal_write is called.
	codegentest.AssertContainsMatch(t, ir, `@promise_write_bytes\(i8\* %sret.*\n(.*\n)*?.*call i64 @pal_write\(i32 %`)
}
...
func TestReadBytesBody(t *testing.T) {
	...
	`)
	codegentest.AssertContains(t, ir, "define void @promise_read_bytes(i8* %sret, i8* %fd, i8* %buf)")
	codegentest.AssertContainsMatch(t, ir, `@promise_read_bytes\(i8\* %sret.*\n(.*\n)*?.*call i64 @pal_file_read\(i32 %`)
}
...
func TestStderrGetterUsesFd2(t *testing.T) {
	...
	codegentest.AssertContains(t, ir, "%promise__OutputStream_v = type { i8*, i64 }")
	// The stderr getter stores fd 2 into the value.
	codegentest.AssertContains(t, ir, "define { i8*, i8* } @__mod_std_stderr()")
	codegentest.AssertContains(t, ir, "insertvalue %promise__OutputStream_v %0, i64 2, 1")
	// Boxed to the Writer view (structural interface fat pointer).
	codegentest.AssertContains(t, ir, "@promise_vtable__OutputStream_as_Writer")
}
...
func TestStdoutGetterUsesFd1(t *testing.T) {
	ir := codegentest.GenerateIR(t, `
		main!() { stdout.write_line("y")?^; }
	`)
	codegentest.AssertContains(t, ir, "define { i8*, i8* } @__mod_std_stdout()")
	codegentest.AssertContains(t, ir, "insertvalue %promise__OutputStream_v %0, i64 1, 1")
}
...
func TestStdinGetterUsesFd0(t *testing.T) {
	...
	`)
	codegentest.AssertContains(t, ir, "%promise__InputStream_v = type { i8*, i64 }")
	codegentest.AssertContains(t, ir, "define { i8*, i8* } @__mod_std_stdin()")
	codegentest.AssertContains(t, ir, "insertvalue %promise__InputStream_v %0, i64 0, 1")
	codegentest.AssertContains(t, ir, "@promise_vtable__InputStream_as_Reader")
}
...
func TestPrintStringStillFd1(t *testing.T) {
	ir := codegentest.GenerateIR(t, `
		main() { print_line("hi"); }
	`)
	codegentest.AssertContains(t, ir, "define void @promise_print_string(i8* %s)")
