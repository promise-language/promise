...
| ~~Generic free-function/method-instance ownership~~ — **Fixed.** `sort__int`, `scan__int`, `sort__Priority`, `Iterator__int.map__int` etc. were module-owned but not instance-owned, so `SplitModuleIRs` left their bodies in the module IR, making it unstable across compilations and causing undefined-symbol linker errors when a cached module BC was reused for a different caller. Fixed in `declareMonoFuncs` and `declareMonoMethodInstances` (mono.go): when `compilingModule != ""`, now also adds `instanceOwnedFuncs[name] = name` so each gets its own `.bc`. Module IR is now stable (one cache entry per module version across all 187 test files). | 9 | ~~Medium~~ Resolved |
