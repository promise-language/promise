...
func generateNetIR(t *testing.T, userSrc string) string {
	t.Helper()
	return generateIRWithCatalogModule(t, "net", netModuleSource(t), userSrc)
}
...
func TestNetResolveHandsOffP(t *testing.T) {
	...
	body := defBody(t, ir, "define void @promise_net_resolve(")
...
func TestNetSocketConnectResolvedHandsOffP(t *testing.T) {
	...
	body := defBody(t, ir, "define void @promise_net_socket_connect_resolved(")
	assertContains(t, body, "@promise_sched_enter_syscall")
	assertContains(t, body, "@pal_socket_connect_resolved(")
	assertContains(t, body, "@promise_sched_exit_syscall")
}
...
func TestNetAddrListDropFreesResolveList(t *testing.T) {
	...
	body := defBody(t, ir, "define void @__mod_net__AddrList.drop(")
	assertContains(t, body, "@promise_net_resolve_free(")

	// The bridge that drop reaches has to call through to the PAL free.
	freeBody := defBody(t, ir, "define void @promise_net_resolve_free(")
	assertContains(t, freeBody, "@pal_resolve_free(")
}
...
func TestNetConnectUsesResolvedFamily(t *testing.T) {
	...
	body := defBody(t, ir, "define { i1, { i8*, i8* }, i8* } @__mod_net_TcpStream.connect(")
	assertContains(t, body, "@promise_net_resolve_family(")
	assertContains(t, body, "@promise_net_resolve_next(")
...
func TestNetResolveAddressReturnsEmptyOnFailure(t *testing.T) {
	...
	body := defBody(t, ir, "define void @promise_net_resolve_address(")
	errBlk := body[strings.Index(body, "\n.err:"):]
	assertContains(t, errBlk, "@promise_string_new(i8* null, i64 0)")
	...
	assertContains(t, okBlk, "sext i32")
}
...
func TestNetConnectAdvancesPastEveryFailedAddress(t *testing.T) {
	...
	assertContains(t, body, "@promise_net_socket_close(")
}
