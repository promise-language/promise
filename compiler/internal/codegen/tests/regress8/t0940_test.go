...
func TestT0940MapBoundOwnedLocalDefaultOwnsNone(t *testing.T) {
	...
	`)
	fn := codegentest.ExtractFunction(ir, "__user.demo")
	if fn == "" {
		t.Fatal("could not extract __user.demo")
	}
	// none-path neutralizes the owned-local default's own drop flag.
	codegentest.AssertContainsMatch(t, fn, `elvis\.none\.\d+:[\s\S]*store i1 false, i1\* %b\.dropflag`)
	// The bound flag owns on both paths and is the value stored into %m.dropflag.
	codegentest.AssertContainsMatch(t, fn, elvisBoundOwnsNone)
	codegentest.AssertContainsMatch(t, fn, elvisBoundFlagStore)
	codegentest.AssertContains(t, fn, `call void @"Map[string, int].drop"`)
}
...
func TestT0940HeapUserBoundOwnedLocalDefaultNeutralized(t *testing.T) {
	...
	codegentest.AssertContainsMatch(t, fn, elvisBoundOwnsNone)
	codegentest.AssertContainsMatch(t, fn, elvisBoundFlagStore)
	codegentest.AssertContains(t, fn, "call void @HeapBox.drop")
}
...
func TestT0940BoundMemberAliasDefaultBorrowsNone(t *testing.T) {
	...
	codegentest.AssertContainsMatch(t, fn, `phi i1 \[ true, %elvis\.some\.\d+ \], \[ false, %elvis\.none\.\d+ \]`)
	codegentest.AssertContainsMatch(t, fn, elvisBoundFlagStore)
}
...
func TestT0940GenericContextMapBoundResolvesDropFunc(t *testing.T) {
	ir := codegentest.GenerateIR(t, `
		...
		demo() { wrap(7); }
	`)
	fn := codegentest.ExtractDefine(ir, `"wrap[int]"`)
	if fn == "" {
...
func TestT0940InlineMapBorrowsOnNoneUnchanged(t *testing.T) {
	...
	if strings.Contains(codegentest.NoneBlockOf(t, fn), "store i1 false, i1* %b.dropflag") {
...
func TestT0940BoundVecMemberSourceDupClonePromotesSome(t *testing.T) {
	...
	codegentest.AssertContainsMatch(t, fn, `store i1 true, i1\* %x\.dropflag\s*\n\s*store i1 %\d+, i1\* %x\.dropflag`)
}
