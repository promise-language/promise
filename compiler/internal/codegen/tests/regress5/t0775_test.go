...
func TestT0775_ForceUnwrapMemberSkipsDup(t *testing.T) {
	ir := codegentest.GenerateIR(t, t0775Decls+`
		mfu() int { h := Holder(hb: HBox(n: 42), s: "a".to_upper(), v: ["x".to_upper()]); return (h.hb!).n; }
		main() { _ := mfu(); }
	`)
	fn := codegentest.ExtractFunction(ir, "__user.mfu")
	if fn == "" {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			ir := codegentest.GenerateIR(t, t0775Decls+`
				...
				main() { _ := mh(); }
			`)
			fn := codegentest.ExtractFunction(ir, "__user.mh")
			if fn == "" {
...
func TestT0775_ParenWrappedMemberSource(t *testing.T) {
	t.Run("force_unwrap_skips_dup", func(t *testing.T) {
		ir := codegentest.GenerateIR(t, t0775Decls+`
			...
			main() { _ := pf(); }
		`)
		fn := codegentest.ExtractFunction(ir, "__user.pf")
		if fn == "" {
	...
	t.Run("handler_dups", func(t *testing.T) {
		ir := codegentest.GenerateIR(t, t0775Decls+`
			...
			main() { _ := ph(); }
		`)
		fn := codegentest.ExtractFunction(ir, "__user.ph")
		if fn == "" {
...
func TestT0775_GenericMethodBodyHandlerDups(t *testing.T) {
	...
	fn := codegentest.ExtractFunction(ir, `"GHolder[HBox].feed"`)
	if fn == "" {
