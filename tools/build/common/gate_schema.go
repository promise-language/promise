...
// GateOutputSchema is the machine-readable contract for `bin/gate` JSON output,
// printed verbatim by `bin/gate schema`. It is embedded here (rather than read
// from a doc file at runtime) so the command works regardless of the working
// directory and never depends on a doc path existing on disk. The human-facing
// narrative lives in docs/gate-system.md ("Gate Output Schema"); keep the two in
// sync when the envelope changes. T0763.
const GateOutputSchema = `# Gate output schema (T0763)
