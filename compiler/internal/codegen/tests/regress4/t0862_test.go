...
func TestT0862GenericStructuralDefaultThroughView(t *testing.T) {
	...
	// instance Box__int.
	if codegentest.ExtractFunction(ir, "Counter.put_two") == "" {
		t.Error("expected synthesized Counter.put_two to be emitted")
	}
	p3 := codegentest.ExtractFunction(ir, "Counter.put_three")
	if p3 == "" {
	...
	codegentest.AssertContains(t, p3, "@Counter.put_two")
	...
	ctVtable := codegentest.ExtractGlobal(ir, "promise_vtable_Counter")
	if ctVtable == "" {
		t.Fatal("expected @promise_vtable_Counter to be emitted")
	}
	codegentest.AssertContains(t, ctVtable, "@Counter.put_two")
	codegentest.AssertContains(t, ctVtable, "@Counter.put_three")
}
...
func TestT0862TransitiveGenericInterfaceDefault(t *testing.T) {
	...
	dd := codegentest.ExtractFunction(ir, "Impl.derived_default")
	if dd == "" {
		t.Fatal("expected synthesized Impl.derived_default to be emitted")
	}
	bd := codegentest.ExtractFunction(ir, "Impl.base_default")
	if bd == "" {
	...
	codegentest.AssertContains(t, bd, "i64")
	codegentest.AssertNotContains(t, bd, "{ i1, i8*, i8* }")
	// derived_default chains to base_default by the concrete name.
	codegentest.AssertContains(t, dd, "@Impl.base_default")
	// The concrete vtable used for dispatch through the Derived[int] view must
	// reference both synthesized defaults.
	vt := codegentest.ExtractGlobal(ir, "promise_vtable_Impl")
	if vt == "" {
		t.Fatal("expected @promise_vtable_Impl to be emitted")
	}
	codegentest.AssertContains(t, vt, "@Impl.derived_default")
	codegentest.AssertContains(t, vt, "@Impl.base_default")
}
...
func TestT0862ConcreteOverridesDefault(t *testing.T) {
	...
	`)
	pt := codegentest.ExtractFunction(ir, "Override.put_two")
	if pt == "" {
	...
	codegentest.AssertContains(t, pt, "add")
	vt := codegentest.ExtractGlobal(ir, "promise_vtable_Override")
	if vt == "" {
		t.Fatal("expected @promise_vtable_Override to be emitted")
	}
	codegentest.AssertContains(t, vt, "@Override.put_two")
}
...
func TestT0862GenericDefaultMethodSkipped(t *testing.T) {
	...
	if codegentest.ExtractFunction(ir, "GCounter.put_two") == "" {
	...
	if codegentest.ExtractFunction(ir, "GCounter.wrap") != "" {
		t.Error("generic default method GCounter.wrap should not be synthesized")
	}
	vt := codegentest.ExtractGlobal(ir, "promise_vtable_GCounter")
	if vt == "" {
		t.Fatal("expected @promise_vtable_GCounter to be emitted")
	}
	codegentest.AssertContains(t, vt, "@GCounter.put_two")
}
