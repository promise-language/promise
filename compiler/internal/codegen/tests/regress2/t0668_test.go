...
func TestT0668_HostTaskDropSpinsUsleepNoCoopStep(t *testing.T) {
	...
	faf := codegentest.FindDefinedFunc(ir, `@"Task[int].free_after_done"`)
	if faf == "" {
...
func TestT0668_CoroutineSiteCooperativeJoin(t *testing.T) {
	...
	for _, sig := range joinFns {
		fn := codegentest.FindDefinedFunc(ir, strings.Fields(sig)[len(strings.Fields(sig))-1]+"(")
		if fn == "" {
			continue
		}
...
func TestT0668_NonCoroutineStructFieldUsesLegacyDrop(t *testing.T) {
	...
	boxDrop := codegentest.FindDefinedFunc(ir, "@Box.drop(")
	if boxDrop == "" {
