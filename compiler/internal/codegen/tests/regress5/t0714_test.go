...
func TestT0714_SliceCompoundReadModifyWrite(t *testing.T) {
	...
	`)
	codegentest.AssertContains(t, ir, `call i64 @"Span.[:]"(`)
	codegentest.AssertContains(t, ir, `add i64`)
	codegentest.AssertContains(t, ir, `call void @"Span.[:]="(`)
}
...
func TestT0714_SliceCompoundStringDropsOld(t *testing.T) {
	...
	`)
	codegentest.AssertContains(t, ir, `call i8* @"Span.[:]"(`)
	codegentest.AssertContains(t, ir, `call void @"Span.[:]="(`)
	// Drop-old block guards against leaking the heap string the getter returned.
	codegentest.AssertContains(t, ir, "compound.strdrop")
...
func TestT0714_SliceCompoundHeapUserTypeDropsOld(t *testing.T) {
	...
	`)
	codegentest.AssertContains(t, ir, `call { i8*, i8* } @"Account.[:]"(`)
	// The fresh getter value is dropped (alias-guarded) before the [:]= write.
	codegentest.AssertContains(t, ir, "compound.userdrop")
	codegentest.AssertContains(t, ir, `call void @Money.drop(`)
}
...
func TestT0714_SliceCompoundGenericMethodTypeSubst(t *testing.T) {
	...
	`)
	codegentest.AssertContains(t, ir, `call i64 @"GSpan[bool].[:]"(`)
	codegentest.AssertContains(t, ir, "add i64")
	codegentest.AssertContains(t, ir, `call void @"GSpan[bool].[:]="(`)
}
