...
func TestT0853_ArcBareIntoOptionalWraps(t *testing.T) {
	ir := codegentest.GenerateIR(t, `main() { Ref[int?] a = Ref[int?](5); Ref[int?] b = Ref[int?](none); }`)
...
func TestT0853_MutexBareIntoOptionalWraps(t *testing.T) {
	ir := codegentest.GenerateIR(t, `main() { Mutex[int?] a = Mutex[int?](5); Mutex[int?] b = Mutex[int?](none); }`)
	body := codegentest.ExtractFunc(ir, ".goroutine.main")
	if body == "" {
		t.Fatalf("expected define @.goroutine.main in IR:\n%s", ir)
	}
