...
func TestT0663ChannelStringDropWalksHeapElements(t *testing.T) {
	...
	`)
	codegentest.AssertContains(t, ir, `define void @"Channel[string].drop"(i8* %this)`)
	...
	codegentest.AssertContains(t, dropFn, "chdrop.head")
	codegentest.AssertContains(t, dropFn, "urem i64")
...
func TestT0663ChannelUserTypeDropFreesInstances(t *testing.T) {
	...
	`)
	codegentest.AssertContains(t, ir, `define void @"Channel[Box].drop"(i8* %this)`)
...
func TestT0663NestedChannelDropDispatchesInner(t *testing.T) {
	...
	`)
	codegentest.AssertContains(t, ir, `define void @"Channel[Channel[int]].drop"(i8* %this)`)
	outerDrop := extractDefinedFunc(ir, `define void @"Channel[Channel[int]].drop"(`)
	if outerDrop == "" {
		t.Fatalf("expected Channel[Channel[int]].drop in IR")
	}
	codegentest.AssertContains(t, outerDrop, "chdrop.head")
	// Each buffered inner channel dropped via its own per-element-type drop.
	codegentest.AssertContains(t, outerDrop, `call void @"Channel[int].drop"(`)
}
...
func TestT0663NoLegacyChannelDropSymbol(t *testing.T) {
	...
	`)
	codegentest.AssertNotContains(t, ir, "define void @Channel.drop(")
	codegentest.AssertNotContains(t, ir, "call void @Channel.drop(")
}
...
func TestT0663ChannelFieldReassignDropsOldChannel(t *testing.T) {
	...
	codegentest.AssertContains(t, ir, "field.chdrop")
	codegentest.AssertContains(t, ir, `call void @"Channel[string].drop"(`)
	codegentest.AssertNotContains(t, ir, "call void @Channel.drop(")
}
