...
- `collectMonoFuncInstances(info)` / `collectMonoMethodInstances(info)` — deduplicate `FuncInstance` (e.g., `sort[int]`) and `MethodInstance` (e.g., `Box[int].transform[string]`) records from `info.FuncInstances` / `info.MethodInstances`.
...
- **`collectMonoFuncInstancesWithExtra` / `collectMonoMethodInstancesWithExtra`**: same pattern — filter extra instances by whether the function/owner-type name is in `modFile`'s declarations.
