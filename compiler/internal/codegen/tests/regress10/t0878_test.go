...
func TestT0878_UnaryOperatorOnThisReceiver(t *testing.T) {
	...
	`)
	body := codegentest.ExtractFunction(ir, "Neg.twice")
	if body == "" {
...
func TestT0878_EnumUnaryOperatorThisInGenericMethod(t *testing.T) {
	ir := codegentest.GenerateIR(t, `
		...
		main() { caller(); }
	`)
	body := codegentest.ExtractFunction(ir, "\"Opt[int].double_neg\"")
	if body == "" {
...
func TestT0878_VirtualUnaryOperatorDispatch(t *testing.T) {
	...
	`)
	body := codegentest.ExtractFunction(ir, "VOpBase.neg_via")
	if body == "" {
