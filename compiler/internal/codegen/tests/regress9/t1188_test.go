...
func TestT1188OwnedOptionalParamBorrowRetainsTemp(t *testing.T) {
	borrowIR := codegentest.GenerateIR(t, `
		type D { int x; }
		f0(D? o) int { return 0; }
		caller() int { return f0(D(x: 1)); }
	`)
	caller := codegentest.ExtractFunc(borrowIR, "__user.caller")
	if caller == "" {
	...
	codegentest.AssertContains(t, caller, "@pal_free")

	moveIR := codegentest.GenerateIR(t, `
		type D { int x; }
		f0(D? move o) int { return 0; }
		caller() int { return f0(D(x: 1)); }
	`)
	callerMove := codegentest.ExtractFunc(moveIR, "__user.caller")
	if callerMove == "" {
		t.Fatalf("caller function not found in IR:\n%s", moveIR)
	}
	...
	moveNamedIR := codegentest.GenerateIR(t, `
		type D { int x; drop(~this){} }
		f0m(D? move o) int { return 0; }
		caller() int { d := D(x: 5); return f0m(move d); }
	`)
	callerNamed := codegentest.ExtractFunc(moveNamedIR, "__user.caller")
	if callerNamed == "" {
		t.Fatalf("caller function not found in IR:\n%s", moveNamedIR)
	}
