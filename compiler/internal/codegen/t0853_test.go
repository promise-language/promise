...
// T0853: constructing an Arc[T?] / Mutex[T?] with a bare (non-optional) value
...
// `Arc[T?](none)` was the same panic shape — c.targetType was never set so
...
// TestT0853_ArcBareIntoOptionalWraps — `Arc[int?](5)` must build a {i1,i64}
// aggregate (wrapOptional) and store THAT into the Arc value slot; the
// bare-i64-store panic shape must be gone. `Arc[int?](none)` must lower to a
...
func TestT0853_ArcBareIntoOptionalWraps(t *testing.T) {
	ir := generateIR(t, `main() { Arc[int?] a = Arc[int?](5); Arc[int?] b = Arc[int?](none); }`)
	body := extractFunc(ir, ".goroutine.main")
	if body == "" {
		t.Fatalf("expected define @.goroutine.main in IR:\n%s", ir)
	}
	// `Arc[int?](5)` → wrapOptional builds the {i1,i64} aggregate.
	if !strings.Contains(body, "insertvalue { i1, i64 } undef, i1 true, 0") {
		t.Errorf("expected wrapOptional aggregate for bare-int Arc[int?](5):\n%s", body)
	}
	...
	// `Arc[int?](none)` → genNoneLit under the newly-set c.targetType emits a
	// zeroinitializer {i1,i64} (NOT the `i1 0` void-optional fallback).
	if !strings.Contains(body, "store { i1, i64 } zeroinitializer") {
		t.Errorf("expected `store { i1, i64 } zeroinitializer` for `Arc[int?](none)`:\n%s", body)
	}
}
...
// Arc[int?] from an already-typed `int?` must NOT re-wrap (skipped via the
