...
| ~~B0043~~ | ~~`for k, v in map[K, Enum]` crashes~~ — **Fixed.** Same root cause as B0030 — resolved by the enum field layout fix. For-in iteration, `keys()`, `values()`, and `entries()` all work correctly for `map[K, Enum]` with both fieldless and data enums. Also validated the T0001 target pattern: `map[string, RecursiveEnum]` inside an enum variant (construct, lookup, for-in, nested). 13 tests in `enum_map_iteration_test.pr`. | Codegen | ~~High~~ Resolved |
