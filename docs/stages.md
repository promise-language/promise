...
| ~~B0027~~ | ~~`os.execute` reads stdout/stderr sequentially — deadlock if child writes >64KB to stderr while stdout pipe is not drained~~ — **Fixed.** Decomposed `pal_execute` into `pal_spawn` + `pal_read_pipe` + `pal_wait_pid`. Added `pal_read_pipe_pair` that reads both pipes concurrently via pthread (POSIX) or sequentially (WASM/Windows stubs). `execute()` in os.pr uses the pair read — no goroutines needed, works in batch tests. | E | ~~Medium~~ Resolved |
...
| B0043 | `for k, v in map[K, Enum]` crashes — for-in iteration on a map whose value type is an enum panics in `genForInMap`: `undeclared map keys/values method for Map[K, Enum]`. Basic map operations (construction, assignment, lookup) work after B0030 fix, but iteration does not. Blocks T0001 (JsonValue with `map[string, JsonValue]` Object variant). Workaround: use parallel arrays (`string[] keys, Enum[] vals`) and index-based iteration. | Codegen | High |
...
|---|------|-----------|----------|
| T0001 | `JsonValue` use `map[string, JsonValue]` for Object variant instead of parallel arrays | B0043 (`for k, v in map[K, Enum]` iteration) | High |
