...
func TestT1388_CloseErrEarlyExitCleansOuterPrefix(t *testing.T) {
	...
	codegentest.AssertContainsMatch(t, ir, `(?s)close\.err\.ret\.\d+:\s*%\d+ = load i1, i1\* %d\.dropflag`)
	...
	codegentest.AssertContains(t, ir, "call void @Drp.drop")
}
...
func TestT1388_CloseErrRetDoesNotImmediatelyReturn(t *testing.T) {
	...
	codegentest.AssertNotContainsMatch(t, ir, `(?s)close\.err\.ret\.\d+:\s*%\d+ = load i8\*, i8\*\* %close\.err\.val`)
}
