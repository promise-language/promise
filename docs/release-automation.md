...
| `wasmtime`, `node` | host platform | **Done** (T2169): the runtimes that EXECUTE a `wasm32-wasi` / `wasm32-web` module, sliced one executable apiece from the pinned upstream release archives. Host dependencies, so the manifest name carries no arch (`runtime-wasmtime`, `runtime-node`) — one host manifest describes one of each. In no variant, `all` included: only a run that targets wasm wants one, so they are fetched on demand. The Node harness script itself is embedded in the compiler, not a blob. |
...
jobs:
  test:
    ...
    steps:
      ...
      # No wasmtime or Node install step: both WASM test runtimes are pinned
      # prebuilts too, staged on demand by the wasm gates and resolved by the
      # compiler from the same store (T2169). A runner's own Node is ignored.
...
- **Default** — dispatch on the current branch, after verifying your local HEAD **is** that branch's pushed tip, so CI tests the commit you are on. It errors if you have not pushed (`--force` dispatches on the remote tip anyway).
- **`--commit <commit>`** — pin CI to a commit-ish (hash, branch, tag, `HEAD~2`; peeled with `rev-parse <ref>^{commit}`). The resolved SHA must be reachable from some `origin` ref — CI checks the commit out from origin, so it has to be pushed — and `ci` then pushes an immutable `ci-pin-<short-sha>` tag there and dispatches on that tag. This is how you get CI at a *past* commit while `main` keeps moving (the release-cut scenario). There is no `--ref` flag: `--commit <branch>` already resolves a branch to its tip.

*Pin-tag lifetime (T1489).* The pin ref must outlive the dispatch call, or the job's checkout fails with `fatal: couldn't find remote ref refs/tags/ci-pin-…`. The rule is **delete only on evidence the run concluded**: with `--watch`, `ci` deletes the tag once every watched platform has a conclusion (green or red) — but *not* when the watch itself gives up (3h ceiling, or a failed status query), because that run may still be queued and about to check the ref out. Otherwise the tag stays on origin and the **next** pinned dispatch prunes it — a `ci-pin-*` is collected only on positive evidence its run completed (≥1 completed `ci.yml` run on that ref and none queued/in-progress), so a pin whose run has not surfaced in the API yet is never collected out from under it. `cut --commit --run-ci` shares the same pin naming, origin-reachability precondition and prune (`pinTagName` / `requirePinnableCommit` / `prunePinTags` in `release_ci.go`), so the two commands never collect each other's live pins.

*Concurrency guards.* `ci.yml` sets `concurrency: {group: ci-${{ github.ref }}, cancel-in-progress: true}`, so any two runs sharing a ref cancel each other. `ci` therefore:
...
5. **Pre-release, then stable — same hash.** Cut `epoch-next` at the release commit and confirm its `release.yml` run is green (this validates the full blobs→manifest→thin→full→verify→publish chain on the exact tree), then cut stable at the *same commit* `epoch-next` validated (gate 8). Because `origin/main` keeps moving under continuous development, pin both cuts to the validated commit with `--commit <commit>` rather than relying on HEAD:

   ```sh
   bin/release cut next --commit <commit>    # gated; refreshes the moving epoch-next pre-release at <commit>
   # …confirm the epoch-next release.yml run is green…
   bin/release cut stable --commit <commit>  # gated; derives the epoch, tags <commit>, pushes, bumps catalog
...
bin/release changes --commit <commit>  # same range but upper bound pinned to <commit>
```
...
**`--commit`**: pins the upper bound to a commit-ish, peeled to a concrete SHA and validated as HEAD-or-ancestor of HEAD (same value semantics as `ci --commit` / `cut --commit`). Use this to preview the precise range for a commit you intend to pass to `cut --commit <commit>`.
