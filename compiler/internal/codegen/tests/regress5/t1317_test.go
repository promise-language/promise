...
func TestT1317ReturnCallByValueEnumCtorArgDrains(t *testing.T) {
	...
	`)
	fn := codegentest.ExtractFunction(ir, "__user.ret_literal")
	if fn == "" {
...
func TestT1317ReturnEnumCtorItselfMovedOut(t *testing.T) {
	...
	`)
	fn := codegentest.ExtractFunction(ir, "__user.make_full")
	if fn == "" {
	...
	codegentest.AssertNotContains(t, fn, "enum.ctor.drop")
}
...
func TestT1317ReturnCallReturningEnumStillDrainsArg(t *testing.T) {
	...
	`)
	fn := codegentest.ExtractFunction(ir, "__user.ret_relay")
	if fn == "" {
...
func TestT1317ReturnMatchEnumArmsMovedOut(t *testing.T) {
	...
	`)
	fn := codegentest.ExtractFunction(ir, "__user.make_via_match")
	if fn == "" {
...
func TestT1317ReturnGenericEnumCtorArgDrains(t *testing.T) {
	...
	`)
	fn := codegentest.ExtractFunction(ir, "__user.ret_generic")
	if fn == "" {
