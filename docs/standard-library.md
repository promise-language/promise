...
| `web` | `modules/web/web.pr` | 195 | Browser host callbacks for `wasm32-web`, entirely `` `target(web) `` — referencing it from any other target is a compile error, not a link failure (see [web-apps.md](web-apps.md), #25). `web.events(el, kinds)` / `events_with_policy(el, kinds, capacity, policy)` subscribe to DOM events, delivered over a real `Channel[i32]` under a bounded queue with an overflow policy (`DropOldest`/`DropNewest`/`Coalesce`) and a `dropped` count; `for e in subscription` is ordinary channel code, nothing new underneath it. `web.on(el, kind, handler)` is sugar: subscribe, then spawn a goroutine that dispatches each event to a `move`d closure. `Element`, `Document` (`document.body`), `Console` (`console.log`), `Event` handles. 2 tests. |
...
**Malformed requests.** A connection that yields no request is not always the
same event, and the server distinguishes two cases. A peer that reaches
end-of-stream having sent nothing — an idle keep-alive connection the client
dropped, or the wake connection `ServerShutdownHandle.shutdown` opens to break
`accept()` — is owed nothing, and the socket is closed in silence: there is no
reader left, and writing on the wake connection would only be noise. A peer that
sent bytes which never became a valid request — an unparseable request line, an
unknown method, or a header block truncated by a half-close — is a live client
waiting on a diagnosis, so it gets `400 Bad Request` with `Connection: close`
and the parse error as the body, and only then the close. A transport error
takes the silent path too, since a broken transport cannot carry the reply.
