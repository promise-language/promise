...
func TestT1264_EnumVecClosureMapReadNoDup(t *testing.T) {
	ir := codegentest.GenerateIR(t, `
		...
		main() { probe(); }
	`)
	fn := codegentest.ExtractDefine(ir, `"Map[int, E].[]"`)
	if fn == "" {
...
func TestT1264_GenericEnumClosureContainerMapReadNoDup(t *testing.T) {
	ir := codegentest.GenerateIR(t, `
		...
		main() { probe(); }
	`)
	fn := codegentest.ExtractDefine(ir, `"Map[int, Box[int]].[]"`)
	if fn == "" {
...
func TestT1264_EnumStringVecMapReadDups(t *testing.T) {
	ir := codegentest.GenerateIR(t, `
		...
		main() { probe(); }
	`)
	fn := codegentest.ExtractDefine(ir, `"Map[int, EW].[]"`)
	if fn == "" {
		t.Fatalf("Map[int, EW].[] not found in IR")
	}
	codegentest.AssertContains(t, fn, "enumdup")
}
...
func TestT1264_EnumRefMapReadDups(t *testing.T) {
	ir := codegentest.GenerateIR(t, `
		...
		main() { probe(); }
	`)
	fn := codegentest.ExtractDefine(ir, `"Map[int, ER].[]"`)
	if fn == "" {
