...
func TestT0680_WasmWebNanotimeImportsHostMonotonic(t *testing.T) {
	...
	codegentest.AssertContains(t, ir, `"wasm-import-module"="promise_env"`)
	codegentest.AssertContains(t, ir, `"wasm-import-name"="monotonic_nanos"`)
	codegentest.AssertContains(t, ir, "call i64 @promise_env.monotonic_nanos()")
...
func TestT0680_NativeNanotimeUnchanged(t *testing.T) {
	...
	codegentest.AssertContains(t, ir, "@clock_gettime")
	if strings.Contains(ir, "@clock_time_get") {
...
func TestT0680_CoopStepEnforcesDeadline(t *testing.T) {
	...
	codegentest.AssertContains(t, ir, "@__promise_test_deadline = global i64 0")
	codegentest.AssertContains(t, ir, "@__promise_test_timed_out = global i8 0")

	step := codegentest.FindDefinedFunc(ir, "@promise_sched_coop_step(")
	if step == "" {
	...
	run := codegentest.FindDefinedFunc(ir, "@promise_sched_coop_run(")
	if run == "" {
...
func TestT0680_WasmTaskReceiveHasTimeoutMerge(t *testing.T) {
	ir := codegentest.GenerateIRForTarget(t, `
		...
		main() { int _x = foo(); }
	`, "wasm32-wasi")
	foo := codegentest.FindDefinedFunc(ir, "@__user.foo(")
	if foo == "" {
...
func TestT0680_WasmTaskDropBreaksSpinOnTimeout(t *testing.T) {
	...
	`, "wasm32-wasi")
	drop := codegentest.FindDefinedFunc(ir, `@"Task[int].drop"`)
	if drop == "" {
